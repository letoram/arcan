...
local function awbwnd_reorder(self, orderv)
	order_image(self.bordert, orderv+1);
	order_image(self.borderd, orderv+1);
	order_image(self.borderl, orderv+1);
	order_image(self.borderr, orderv+1);
	order_image(self.canvas, orderv);
	for key, val in pairs(self.directions) do
			val:reorder(orderv+1);
	end
end
