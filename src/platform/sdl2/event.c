...
static inline void process_axismotion(
	arcan_platform_evctx ctx, const SDL_JoyAxisEvent* const ev)
{
	...
	if (process_axis(ctx, daxis, ev->value, &dstv)){
		...
		arcan_platform_event_enqueue(ctx, &newevent);
	}
}
...
static inline void process_mousemotion(arcan_platform_evctx ctx,
	const SDL_MouseMotionEvent* const ev)
{
	...
	if (mouse_relative){
		...
		arcan_platform_event_enqueue(ctx, &nev);
		nev.io.subid = 1;
...
static inline void process_hatmotion(
	arcan_platform_evctx ctx, unsigned devid, unsigned hatid, unsigned value)
{
 ...
 * do it here, we need an explicit callback trigger via the SDL_AddEventWatch */
		...
		case SDL_WINDOWEVENT:
			if (event.window.event == SDL_WINDOWEVENT_RESIZED ||
				...
				arcan_platform_event_enqueue(ctx, &(arcan_event){
...
void drop_joytbl(arcan_platform_evctx ctx)
{
	for(int i=0; i < iodev.n_joy; i++){
		if (!iodev.joys[i].tagged){
			...
			arcan_platform_event_enqueue(ctx, &remev);
...
void arcan_platform_event_keyrepeat(arcan_platform_evctx ctx, int* rate, int* delay)
{
...
void arcan_platform_event_init(arcan_platform_evctx ctx)
{
	...
	if (!first_init){
		...
		arcan_platform_event_analogfilter(-1, 0,
			-32768, 32767, 0, 1, ARCAN_ANALOGFILTER_PASS);
		arcan_platform_event_analogfilter(-1, 1,
			-32768, 32767, 0, 1, ARCAN_ANALOGFILTER_PASS);
		first_init = true;
		int r = 0, d = 0;
		arcan_platform_event_keyrepeat(ctx, &r, &d);
	}
