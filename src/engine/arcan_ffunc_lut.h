 ...
 *
 * Description: This header is included as part of video.h but involves
 * the Lua layer, the frameservers and the 3d path as well. The approach
 * is to avoid the use of direct callbacks and instead use an 8-bit
 * read-only indexed lookup-table of pre-registered functions.
 */
...
/*
 * all permitted ffuncs should be added to this enum and then
 * explicitly mapped in the underlying _ffunc_lut.c file in initlut()
 */
enum arcan_ffunc {
/* arcan_ffunc_lut.c */
	FFUNC_FATAL = 0,
...
/*
 * This is used in the defined prototypes and the defined
 * implementation for each ffunc to ensure system name use and trigger
 * compile errors if we ever need to change the prototype as the behavior
 * is sensitive, e.g.
