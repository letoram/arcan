// Compiler builtins used by Lua 5.4 parser (translate-c output, cleaned up)
...
// Opaque struct stubs referenced by type aliases in macro section (lines 6851+)
...
fn strcmp(a: [*c]const u8, b: [*c]const u8) c_int {
    var i: usize = 0;
...
fn strncpy(dst: [*c]u8, src: [*c]const u8, n: c_ulong) [*c]u8 {
    var i: usize = 0;
...
fn strncat(dst: [*c]u8, src: [*c]const u8, n: c_ulong) [*c]u8 {
    // Find end of dst
    var dlen: usize = 0;
    while (dst[dlen] != 0) : (dlen += 1) {}
    // Append up to n chars from src
    var i: usize = 0;
    while (i < n and src[i] != 0) : (i += 1) dst[dlen + i] = src[i];
    dst[dlen + i] = 0;
    return dst;
}
...
fn realloc(ptr: ?*anyopaque, size: usize) ?*anyopaque {
    _ = ptr;
    _ = size;
    return null; // stub — parser doesn't actually call realloc at runtime in our usage
}

fn strtod(s: [*c]const u8, endptr: ?*?[*c]const u8) f64 {
    if (endptr) |ep| ep.* = s;
    const str: [*:0]const u8 = @ptrCast(s);
    const slice = std.mem.span(str);
    const result = std.fmt.parseFloat(f64, slice) catch return 0.0;
    if (endptr) |ep| {
        // Advance past consumed characters
        var i: usize = 0;
...
pub const struct_BlockCnt = extern struct {
    previous: [*c]struct_BlockCnt = @import("std").mem.zeroes([*c]struct_BlockCnt),
    firstlabel: c_int = @import("std").mem.zeroes(c_int),
    firstgoto: c_int = @import("std").mem.zeroes(c_int),
    nactvar: lu_byte = @import("std").mem.zeroes(lu_byte),
    upval: lu_byte = @import("std").mem.zeroes(lu_byte),
    isloop: lu_byte = @import("std").mem.zeroes(lu_byte),
    insidetbc: lu_byte = @import("std").mem.zeroes(lu_byte),
};
pub const struct_FuncState = extern struct {
    ...
    bl: [*c]struct_BlockCnt = @import("std").mem.zeroes([*c]struct_BlockCnt),
    pc: c_int = @import("std").mem.zeroes(c_int),
...
pub export fn luaY_nvarstack(fs: [*c]FuncState) c_int {
    return reglevel(fs, @as(c_int, @bitCast(@as(c_uint, fs.*.nactvar))));
}
...
pub export fn luaY_parser(L: [*c]lua_State, z: [*c]ZIO, buff: [*c]Mbuffer, dyd: [*c]Dyndata, name: [*c]const u8, firstchar: c_int) [*c]LClosure {
    var lexstate: LexState = undefined;
    var funcstate: FuncState = undefined;
    const cl: [*c]LClosure = luaF_newLclosure(L, 1);
    {
        const io: [*c]TValue = &L.*.top.p.*.val;
        io.*.value_.gc = &@as([*c]union_GCUnion, @ptrCast(@alignCast(cl))).*.gc;
        io.*.tt_ = @as(lu_byte, @bitCast(@as(i8, @truncate((6 | (0 << @intCast(4))) | (1 << @intCast(6))))));
        _ = if (!((@as(c_int, @bitCast(@as(c_uint, io.*.tt_))) & (1 << @intCast(6))) != 0) or (((@as(c_int, @bitCast(@as(c_uint, io.*.tt_))) & 63) == @as(c_int, @bitCast(@as(c_uint, io.*.value_.gc.*.tt)))) and ((L == @as([*c]lua_State, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(0)))))) or !((@as(c_int, @bitCast(@as(c_uint, io.*.value_.gc.*.marked))) & (@as(c_int, @bitCast(@as(c_uint, L.*.l_G.*.currentwhite))) ^ ((1 << @intCast(3)) | (1 << @intCast(4))))) != 0)))) 0 else 0;
    }
    luaD_inctop(L);
    lexstate.h = luaH_new(L);
    {
        const io: [*c]TValue = &L.*.top.p.*.val;
        io.*.value_.gc = &@as([*c]union_GCUnion, @ptrCast(@alignCast(lexstate.h))).*.gc;
        io.*.tt_ = @as(lu_byte, @bitCast(@as(i8, @truncate((5 | (0 << @intCast(4))) | (1 << @intCast(6))))));
        _ = if (!((@as(c_int, @bitCast(@as(c_uint, io.*.tt_))) & (1 << @intCast(6))) != 0) or (((@as(c_int, @bitCast(@as(c_uint, io.*.tt_))) & 63) == @as(c_int, @bitCast(@as(c_uint, io.*.value_.gc.*.tt)))) and ((L == @as([*c]lua_State, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(0)))))) or !((@as(c_int, @bitCast(@as(c_uint, io.*.value_.gc.*.marked))) & (@as(c_int, @bitCast(@as(c_uint, L.*.l_G.*.currentwhite))) ^ ((1 << @intCast(3)) | (1 << @intCast(4))))) != 0)))) 0 else 0;
    }
    luaD_inctop(L);
    funcstate.f = blk: {
        const tmp = luaF_newproto(L);
        cl.*.p = tmp;
        break :blk tmp;
    };
    _ = if (((@as(c_int, @bitCast(@as(c_uint, cl.*.marked))) & (1 << @intCast(5))) != 0) and ((@as(c_int, @bitCast(@as(c_uint, cl.*.p.*.marked))) & ((1 << @intCast(3)) | (1 << @intCast(4)))) != 0)) luaC_barrier_(L, &@as([*c]union_GCUnion, @ptrCast(@alignCast(cl))).*.gc, &@as([*c]union_GCUnion, @ptrCast(@alignCast(cl.*.p))).*.gc) else 0;
    funcstate.f.*.source = luaS_new(L, name);
    _ = if (((@as(c_int, @bitCast(@as(c_uint, funcstate.f.*.marked))) & (1 << @intCast(5))) != 0) and ((@as(c_int, @bitCast(@as(c_uint, funcstate.f.*.source.*.marked))) & ((1 << @intCast(3)) | (1 << @intCast(4)))) != 0)) luaC_barrier_(L, &@as([*c]union_GCUnion, @ptrCast(@alignCast(funcstate.f))).*.gc, &@as([*c]union_GCUnion, @ptrCast(@alignCast(funcstate.f.*.source))).*.gc) else 0;
    lexstate.buff = buff;
    lexstate.dyd = dyd;
    dyd.*.actvar.n = 0;
    dyd.*.gt.n = 0;
...
pub extern fn luaK_code(fs: [*c]FuncState, i: Instruction) c_int;
pub extern fn luaK_codeABx(fs: [*c]FuncState, o: OpCode, A: c_int, Bx: c_uint) c_int;
pub extern fn luaK_codeAsBx(fs: [*c]FuncState, o: OpCode, A: c_int, Bx: c_int) c_int;
pub extern fn luaK_codeABCk(fs: [*c]FuncState, o: OpCode, A: c_int, B: c_int, C: c_int, k: c_int) c_int;
pub extern fn luaK_isKint(e: [*c]expdesc) c_int;
pub extern fn luaK_exp2const(fs: [*c]FuncState, e: [*c]const expdesc, v: [*c]TValue) c_int;
pub extern fn luaK_fixline(fs: [*c]FuncState, line: c_int) void;
pub extern fn luaK_nil(fs: [*c]FuncState, from: c_int, n: c_int) void;
pub extern fn luaK_reserveregs(fs: [*c]FuncState, n: c_int) void;
pub extern fn luaK_checkstack(fs: [*c]FuncState, n: c_int) void;
pub extern fn luaK_int(fs: [*c]FuncState, reg: c_int, n: lua_Integer) void;
pub extern fn luaK_dischargevars(fs: [*c]FuncState, e: [*c]expdesc) void;
pub extern fn luaK_exp2anyreg(fs: [*c]FuncState, e: [*c]expdesc) c_int;
pub extern fn luaK_exp2anyregup(fs: [*c]FuncState, e: [*c]expdesc) void;
pub extern fn luaK_exp2nextreg(fs: [*c]FuncState, e: [*c]expdesc) void;
pub extern fn luaK_exp2val(fs: [*c]FuncState, e: [*c]expdesc) void;
pub extern fn luaK_exp2RK(fs: [*c]FuncState, e: [*c]expdesc) c_int;
pub extern fn luaK_self(fs: [*c]FuncState, e: [*c]expdesc, key: [*c]expdesc) void;
pub extern fn luaK_indexed(fs: [*c]FuncState, t: [*c]expdesc, k: [*c]expdesc) void;
pub extern fn luaK_goiftrue(fs: [*c]FuncState, e: [*c]expdesc) void;
pub extern fn luaK_goiffalse(fs: [*c]FuncState, e: [*c]expdesc) void;
pub extern fn luaK_storevar(fs: [*c]FuncState, @"var": [*c]expdesc, e: [*c]expdesc) void;
pub extern fn luaK_setreturns(fs: [*c]FuncState, e: [*c]expdesc, nresults: c_int) void;
pub extern fn luaK_setoneret(fs: [*c]FuncState, e: [*c]expdesc) void;
pub extern fn luaK_jump(fs: [*c]FuncState) c_int;
pub extern fn luaK_ret(fs: [*c]FuncState, first: c_int, nret: c_int) void;
pub extern fn luaK_patchlist(fs: [*c]FuncState, list: c_int, target: c_int) void;
pub extern fn luaK_patchtohere(fs: [*c]FuncState, list: c_int) void;
pub extern fn luaK_concat(fs: [*c]FuncState, l1: [*c]c_int, l2: c_int) void;
pub extern fn luaK_getlabel(fs: [*c]FuncState) c_int;
pub extern fn luaK_prefix(fs: [*c]FuncState, op: UnOpr, v: [*c]expdesc, line: c_int) void;
pub extern fn luaK_infix(fs: [*c]FuncState, op: BinOpr, v: [*c]expdesc) void;
pub extern fn luaK_posfix(fs: [*c]FuncState, op: BinOpr, v1: [*c]expdesc, v2: [*c]expdesc, line: c_int) void;
pub extern fn luaK_settablesize(fs: [*c]FuncState, pc: c_int, ra: c_int, asize: c_int, hsize: c_int) void;
pub extern fn luaK_setlist(fs: [*c]FuncState, base: c_int, nelems: c_int, tostore: c_int) void;
pub extern fn luaK_finish(fs: [*c]FuncState) void;
pub extern fn luaK_semerror(ls: [*c]LexState, msg: [*c]const u8) noreturn;
...
pub fn statement(ls: [*c]LexState) callconv(.c) void {
    const line: c_int = ls.*.linenumber;
    luaE_incCstack(ls.*.L);
    switch (ls.*.t.token) {
        59 => {
            luaX_next(ls);
    ...
    ls.*.fs.*.freereg = @as(lu_byte, @bitCast(@as(i8, @truncate(luaY_nvarstack(ls.*.fs)))));
    ls.*.L.*.nCcalls -%= 1;
}
pub fn expr(ls: [*c]LexState, v: [*c]expdesc) callconv(.c) void {
    _ = subexpr(ls, v, 0);
}
pub fn error_expected(ls: [*c]LexState, token: c_int) callconv(.c) noreturn {
    {
        var fmtbuf: [512]u8 = undefined;
        const result = std.fmt.bufPrintZ(&fmtbuf, "{s} expected", .{
            std.mem.span(@as([*:0]const u8, @ptrCast(luaX_token2str(ls, token)))),
        }) catch "? expected";
        luaX_syntaxerror(ls, lua_pushstring(ls.*.L, result));
    }
}
pub fn errorlimit(fs: [*c]FuncState, limit: c_int, what: [*c]const u8) callconv(.c) noreturn {
    const L: [*c]lua_State = fs.*.ls.*.L;
    const line: c_int = fs.*.f.*.linedefined;
    var where_buf: [128]u8 = undefined;
    const where: [*c]const u8 = if (line == 0) "main function" else blk: {
        const w = std.fmt.bufPrintZ(&where_buf, "function at line {d}", .{line}) catch "function at line ?";
        break :blk lua_pushstring(L, w);
    };
    var msg_buf: [512]u8 = undefined;
    const msg: [*c]const u8 = blk: {
        const m = std.fmt.bufPrintZ(&msg_buf, "too many {s} (limit is {d}) in {s}", .{
            std.mem.span(@as([*:0]const u8, @ptrCast(what))),
            limit,
            std.mem.span(@as([*:0]const u8, @ptrCast(where))),
        }) catch "too many ? (limit is ?) in ?";
        break :blk lua_pushstring(L, m);
    };
    luaX_syntaxerror(fs.*.ls, msg);
}
pub fn checklimit(fs: [*c]FuncState, v: c_int, l: c_int, what: [*c]const u8) callconv(.c) void {
    if (v > l) {
        errorlimit(fs, l, what);
    }
}
pub fn testnext(ls: [*c]LexState, c: c_int) callconv(.c) c_int {
    if (ls.*.t.token == c) {
        luaX_next(ls);
        return 1;
    } else return 0;
}
pub fn check(ls: [*c]LexState, c: c_int) callconv(.c) void {
    if (ls.*.t.token != c) {
        error_expected(ls, c);
    }
}
pub fn checknext(ls: [*c]LexState, c: c_int) callconv(.c) void {
    check(ls, c);
    luaX_next(ls);
}
pub fn check_match(ls: [*c]LexState, what: c_int, who: c_int, where: c_int) callconv(.c) void {
    if (__builtin_expect(@as(c_long, @intFromBool(@intFromBool(!(testnext(ls, what) != 0)) != 0)), @as(c_long, 0)) != 0) {
        if (where == ls.*.linenumber) {
            error_expected(ls, what);
        } else {
            {
                var fmtbuf: [512]u8 = undefined;
                const result = std.fmt.bufPrintZ(&fmtbuf, "{s} expected (to close {s} at line {d})", .{
                    std.mem.span(@as([*:0]const u8, @ptrCast(luaX_token2str(ls, what)))),
                    std.mem.span(@as([*:0]const u8, @ptrCast(luaX_token2str(ls, who)))),
                    where,
                }) catch "? expected (to close ? at line ?)";
                luaX_syntaxerror(ls, lua_pushstring(ls.*.L, result));
            }
        }
    }
}
pub fn str_checkname(ls: [*c]LexState) callconv(.c) [*c]TString {
    check(ls, TK_NAME);
    const ts: [*c]TString = ls.*.t.seminfo.ts;
    luaX_next(ls);
    return ts;
}
pub fn init_exp(e: [*c]expdesc, k: expkind, i: c_int) callconv(.c) void {
    e.*.t = -1;
    e.*.f = -1;
    e.*.k = k;
    e.*.u.info = i;
}
pub fn codestring(e: [*c]expdesc, s: [*c]TString) callconv(.c) void {
    e.*.t = -1;
    e.*.f = -1;
    e.*.k = @as(c_uint, @bitCast(VKSTR));
    e.*.u.strval = s;
}
pub fn codename(ls: [*c]LexState, e: [*c]expdesc) callconv(.c) void {
...
pub fn registerlocalvar(ls: [*c]LexState, fs: [*c]FuncState, varname: [*c]TString) callconv(.c) c_int {
    ...
    while (oldsize < f.*.sizelocvars) {
        (blk: {
            const tmp = oldsize;
            oldsize += 1;
            if (tmp >= 0) break :blk f.*.locvars + @as(usize, @intCast(tmp)) else break :blk f.*.locvars - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
        }).*.varname = null;
    }
    (blk: {
        const tmp = fs.*.ndebugvars;
        if (tmp >= 0) break :blk f.*.locvars + @as(usize, @intCast(tmp)) else break :blk f.*.locvars - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
    }).*.varname = varname;
    (blk: {
        const tmp = fs.*.ndebugvars;
    ...
    _ = if (((@as(c_int, @bitCast(@as(c_uint, f.*.marked))) & (1 << @intCast(5))) != 0) and ((@as(c_int, @bitCast(@as(c_uint, varname.*.marked))) & ((1 << @intCast(3)) | (1 << @intCast(4)))) != 0)) luaC_barrier_(ls.*.L, &@as([*c]union_GCUnion, @ptrCast(@alignCast(f))).*.gc, &@as([*c]union_GCUnion, @ptrCast(@alignCast(varname))).*.gc) else 0;
    const result = @as(c_int, @bitCast(@as(c_int, fs.*.ndebugvars)));
    fs.*.ndebugvars += 1;
    return result;
}
pub fn new_localvar(ls: [*c]LexState, name: [*c]TString) callconv(.c) c_int {
    const L: [*c]lua_State = ls.*.L;
    ...
    const @"var": [*c]Vardesc = &(blk: {
        if (idx >= 0) break :blk dyd.*.actvar.arr + @as(usize, @intCast(idx)) else break :blk dyd.*.actvar.arr - ~@as(usize, @bitCast(@as(isize, @intCast(idx)) +% -1));
    }).*;
    @"var".*.vd.kind = 0;
    @"var".*.vd.name = name;
    return (dyd.*.actvar.n - 1) - fs.*.firstlocal;
}
pub fn getlocalvardesc(fs: [*c]FuncState, vidx: c_int) callconv(.c) [*c]Vardesc {
    return &(blk: {
        const tmp = fs.*.firstlocal + vidx;
...
pub fn reglevel(fs: [*c]FuncState, arg_nvar: c_int) callconv(.c) c_int {
    var nvar = arg_nvar;
    while (blk: {
        const tmp = nvar;
        nvar -= 1;
        ...
        const vd: [*c]Vardesc = getlocalvardesc(fs, nvar);
        if (@as(c_int, @bitCast(@as(c_uint, vd.*.vd.kind))) != 3) return @as(c_int, @bitCast(@as(c_uint, vd.*.vd.ridx))) + 1;
    }
    return 0;
}
pub fn localdebuginfo(fs: [*c]FuncState, vidx: c_int) callconv(.c) [*c]LocVar {
    const vd: [*c]Vardesc = getlocalvardesc(fs, vidx);
    if (@as(c_int, @bitCast(@as(c_uint, vd.*.vd.kind))) == 3) return null else {
        const idx: c_int = @as(c_int, @bitCast(@as(c_int, vd.*.vd.pidx)));
        return &(blk: {
            if (idx >= 0) break :blk fs.*.f.*.locvars + @as(usize, @intCast(idx)) else break :blk fs.*.f.*.locvars - ~@as(usize, @bitCast(@as(isize, @intCast(idx)) +% -1));
        }).*;
    }
}
pub fn init_var(fs: [*c]FuncState, e: [*c]expdesc, vidx: c_int) callconv(.c) void {
    e.*.t = -1;
    e.*.f = -1;
    e.*.k = @as(c_uint, @bitCast(VLOCAL));
    e.*.u.@"var".vidx = @as(c_ushort, @bitCast(@as(c_short, @truncate(vidx))));
    e.*.u.@"var".ridx = getlocalvardesc(fs, vidx).*.vd.ridx;
}
pub fn check_readonly(ls: [*c]LexState, e: [*c]expdesc) callconv(.c) void {
    const fs: [*c]FuncState = ls.*.fs;
    var varname: [*c]TString = null;
    switch (e.*.k) {
        @as(c_uint, @bitCast(@as(c_int, 11))) => {
            varname = (blk: {
                const tmp = e.*.u.info;
                if (tmp >= 0) break :blk ls.*.dyd.*.actvar.arr + @as(usize, @intCast(tmp)) else break :blk ls.*.dyd.*.actvar.arr - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
            }).*.vd.name;
        },
        @as(c_uint, @bitCast(@as(c_int, 9))) => {
            const vardesc: [*c]Vardesc = getlocalvardesc(fs, @as(c_int, @bitCast(@as(c_uint, e.*.u.@"var".vidx))));
            if (@as(c_int, @bitCast(@as(c_uint, vardesc.*.vd.kind))) != 0) {
                varname = vardesc.*.vd.name;
            }
        },
        @as(c_uint, @bitCast(@as(c_int, 10))) => {
            const up: [*c]Upvaldesc = &(blk: {
                const tmp = e.*.u.info;
                if (tmp >= 0) break :blk fs.*.f.*.upvalues + @as(usize, @intCast(tmp)) else break :blk fs.*.f.*.upvalues - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
            }).*;
            if (@as(c_int, @bitCast(@as(c_uint, up.*.kind))) != 0) {
                varname = up.*.name;
            }
        },
        else => return,
    ...
    if (varname != null) {
        var fmtbuf: [512]u8 = undefined;
        const msg: [*c]const u8 = blk: {
            const m = std.fmt.bufPrintZ(&fmtbuf, "attempt to assign to const variable '{s}'", .{
                std.mem.span(@as([*:0]const u8, @ptrCast(@as([*c]u8, @ptrCast(@alignCast(&varname.*.contents[0])))))),
            }) catch "attempt to assign to const variable '?'";
            break :blk lua_pushstring(ls.*.L, m);
        };
        luaK_semerror(ls, msg);
    }
}
pub fn adjustlocalvars(ls: [*c]LexState, nvars: c_int) callconv(.c) void {
    const fs: [*c]FuncState = ls.*.fs;
    var reglevel_1: c_int = luaY_nvarstack(fs);
    var i: c_int = 0;
    while (i < nvars) : (i += 1) {
        const vidx: c_int = @as(c_int, @bitCast(@as(c_uint, fs.*.nactvar)));
        fs.*.nactvar +%= 1;
        const @"var": [*c]Vardesc = getlocalvardesc(fs, vidx);
        @"var".*.vd.ridx = @as(lu_byte, @bitCast(@as(i8, @truncate(reglevel_1))));
        reglevel_1 += 1;
        @"var".*.vd.pidx = @as(c_short, @bitCast(@as(c_short, @truncate(registerlocalvar(ls, fs, @"var".*.vd.name)))));
    }
}
pub fn removevars(fs: [*c]FuncState, tolevel: c_int) callconv(.c) void {
    fs.*.ls.*.dyd.*.actvar.n -= @as(c_int, @bitCast(@as(c_uint, fs.*.nactvar))) - tolevel;
    while (@as(c_int, @bitCast(@as(c_uint, fs.*.nactvar))) > tolevel) {
        fs.*.nactvar -%= 1;
        const @"var": [*c]LocVar = localdebuginfo(fs, @as(c_int, @bitCast(@as(c_uint, fs.*.nactvar))));
        if (@"var" != null) {
            @"var".*.endpc = fs.*.pc;
        }
    }
}
pub fn searchupvalue(fs: [*c]FuncState, name: [*c]TString) callconv(.c) c_int {
    const up: [*c]Upvaldesc = fs.*.f.*.upvalues;
    var i: c_int = 0;
    while (i < @as(c_int, @bitCast(@as(c_uint, fs.*.nups)))) : (i += 1) {
        if ((blk: {
            if (i >= 0) break :blk up + @as(usize, @intCast(i)) else break :blk up - ~@as(usize, @bitCast(@as(isize, @intCast(i)) +% -1));
        }).*.name == name) return i;
    }
    return -1;
}
pub fn allocupvalue(fs: [*c]FuncState) callconv(.c) [*c]Upvaldesc {
    const f: [*c]Proto = fs.*.f;
    ...
    while (oldsize < f.*.sizeupvalues) {
        (blk: {
            const tmp = oldsize;
            oldsize += 1;
            if (tmp >= 0) break :blk f.*.upvalues + @as(usize, @intCast(tmp)) else break :blk f.*.upvalues - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
        }).*.name = null;
    }
    const idx = fs.*.nups;
    fs.*.nups +%= 1;
...
pub fn newupvalue(arg_fs: [*c]FuncState, arg_name: [*c]TString, arg_v: [*c]expdesc) callconv(.c) c_int {
    var fs = arg_fs;
    _ = &fs;
    var name = arg_name;
    _ = &name;
    var v = arg_v;
    _ = &v;
    var up: [*c]Upvaldesc = allocupvalue(fs);
    _ = &up;
    var prev: [*c]FuncState = fs.*.prev;
    _ = &prev;
    if (v.*.k == @as(c_uint, @bitCast(VLOCAL))) {
        up.*.instack = 1;
        up.*.idx = v.*.u.@"var".ridx;
        up.*.kind = getlocalvardesc(prev, @as(c_int, @bitCast(@as(c_uint, v.*.u.@"var".vidx)))).*.vd.kind;
        _ = @as(c_int, 0);
    } else {
        up.*.instack = 0;
        up.*.idx = @as(lu_byte, @bitCast(@as(i8, @truncate(v.*.u.info))));
        up.*.kind = (blk: {
            const tmp = v.*.u.info;
            if (tmp >= 0) break :blk prev.*.f.*.upvalues + @as(usize, @intCast(tmp)) else break :blk prev.*.f.*.upvalues - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
        }).*.kind;
        _ = @as(c_int, 0);
    }
    up.*.name = name;
    _ = if (((@as(c_int, @bitCast(@as(c_uint, fs.*.f.*.marked))) & (@as(c_int, 1) << @intCast(@as(c_int, 5)))) != 0) and ((@as(c_int, @bitCast(@as(c_uint, name.*.marked))) & ((@as(c_int, 1) << @intCast(@as(c_int, 3))) | (@as(c_int, 1) << @intCast(@as(c_int, 4))))) != 0)) luaC_barrier_(fs.*.ls.*.L, blk: {
        _ = @as(c_int, 0);
        break :blk &@as([*c]union_GCUnion, @ptrCast(@alignCast(fs.*.f))).*.gc;
    }, blk: {
        _ = @as(c_int, 0);
        break :blk &@as([*c]union_GCUnion, @ptrCast(@alignCast(name))).*.gc;
    }) else @as(c_int, 0);
    return @as(c_int, @bitCast(@as(c_uint, fs.*.nups))) - @as(c_int, 1);
}
pub fn searchvar(arg_fs: [*c]FuncState, arg_n: [*c]TString, arg_var: [*c]expdesc) callconv(.c) c_int {
    var fs = arg_fs;
    _ = &fs;
    var n = arg_n;
    _ = &n;
    ...
    _ = &i;
    {
        i = @as(c_int, @bitCast(@as(c_uint, fs.*.nactvar))) - @as(c_int, 1);
        while (i >= @as(c_int, 0)) : (i -= 1) {
            var vd: [*c]Vardesc = getlocalvardesc(fs, i);
            _ = &vd;
            if (n == vd.*.vd.name) {
                if (@as(c_int, @bitCast(@as(c_uint, vd.*.vd.kind))) == @as(c_int, 3)) {
                    init_exp(@"var", @as(c_uint, @bitCast(VCONST)), fs.*.firstlocal + i);
                } else {
                    init_var(fs, @"var", i);
                }
                return @as(c_int, @bitCast(@"var".*.k));
            }
        }
    }
    return -@as(c_int, 1);
}
pub fn markupval(arg_fs: [*c]FuncState, arg_level: c_int) callconv(.c) void {
    var fs = arg_fs;
    _ = &fs;
    var level = arg_level;
    _ = &level;
    var bl: [*c]BlockCnt = fs.*.bl;
    _ = &bl;
    while (@as(c_int, @bitCast(@as(c_uint, bl.*.nactvar))) > level) {
        bl = bl.*.previous;
    }
    bl.*.upval = 1;
    fs.*.needclose = 1;
}
pub fn marktobeclosed(arg_fs: [*c]FuncState) callconv(.c) void {
    var fs = arg_fs;
    _ = &fs;
    var bl: [*c]BlockCnt = fs.*.bl;
    _ = &bl;
...
pub fn singlevaraux(arg_fs: [*c]FuncState, arg_n: [*c]TString, arg_var: [*c]expdesc, arg_base: c_int) callconv(.c) void {
    var fs = arg_fs;
    _ = &fs;
    var n = arg_n;
    _ = &n;
    ...
    var base = arg_base;
    _ = &base;
    if (fs == @as([*c]FuncState, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        init_exp(@"var", @as(c_uint, @bitCast(VVOID)), @as(c_int, 0));
    } else {
        var v: c_int = searchvar(fs, n, @"var");
        _ = &v;
        if (v >= @as(c_int, 0)) {
            if ((v == VLOCAL) and !(base != 0)) {
                markupval(fs, @as(c_int, @bitCast(@as(c_uint, @"var".*.u.@"var".vidx))));
            }
        } else {
            var idx: c_int = searchupvalue(fs, n);
            _ = &idx;
            if (idx < @as(c_int, 0)) {
                singlevaraux(fs.*.prev, n, @"var", @as(c_int, 0));
                if ((@"var".*.k == @as(c_uint, @bitCast(VLOCAL))) or (@"var".*.k == @as(c_uint, @bitCast(VUPVAL)))) {
                    idx = newupvalue(fs, n, @"var");
                } else return;
            }
            init_exp(@"var", @as(c_uint, @bitCast(VUPVAL)), idx);
        }
    }
}
pub fn singlevar(arg_ls: [*c]LexState, arg_var: [*c]expdesc) callconv(.c) void {
    var ls = arg_ls;
    _ = &ls;
    var @"var" = arg_var;
    _ = &@"var";
    var varname: [*c]TString = str_checkname(ls);
    _ = &varname;
    var fs: [*c]FuncState = ls.*.fs;
    _ = &fs;
    singlevaraux(fs, varname, @"var", @as(c_int, 1));
    if (@"var".*.k == @as(c_uint, @bitCast(VVOID))) {
        var key: expdesc = undefined;
        _ = &key;
        singlevaraux(fs, ls.*.envn, @"var", @as(c_int, 1));
        _ = @as(c_int, 0);
...
pub fn adjust_assign(arg_ls: [*c]LexState, arg_nvars: c_int, arg_nexps: c_int, arg_e: [*c]expdesc) callconv(.c) void {
    var ls = arg_ls;
    _ = &ls;
    var nvars = arg_nvars;
    _ = &nvars;
    ...
    var fs: [*c]FuncState = ls.*.fs;
    _ = &fs;
    var needed: c_int = nvars - nexps;
    _ = &needed;
    if ((e.*.k == @as(c_uint, @bitCast(VCALL))) or (e.*.k == @as(c_uint, @bitCast(VVARARG)))) {
        var extra: c_int = needed + @as(c_int, 1);
        _ = &extra;
        if (extra < @as(c_int, 0)) {
            extra = 0;
        }
        ...
        if (e.*.k != @as(c_uint, @bitCast(VVOID))) {
            luaK_exp2nextreg(fs, e);
        }
        if (needed > @as(c_int, 0)) {
            luaK_nil(fs, @as(c_int, @bitCast(@as(c_uint, fs.*.freereg))), needed);
        }
    }
    if (needed > @as(c_int, 0)) {
        luaK_reserveregs(fs, needed);
    } else {
        fs.*.freereg +%= @as(lu_byte, @bitCast(@as(i8, @truncate(needed))));
    }
}
pub fn jumpscopeerror(arg_ls: [*c]LexState, arg_gt: [*c]Labeldesc) callconv(.c) noreturn {
    var ls = arg_ls;
    _ = &ls;
    var gt = arg_gt;
    _ = &gt;
    var varname: [*c]const u8 = @as([*c]u8, @ptrCast(@alignCast(&getlocalvardesc(ls.*.fs, @as(c_int, @bitCast(@as(c_uint, gt.*.nactvar)))).*.vd.name.*.contents[@as(usize, @intCast(0))])));
    _ = &varname;
    var fmtbuf: [512]u8 = undefined;
    var msg: [*c]const u8 = undefined;
    _ = &msg;
    msg = blk: {
        const m = std.fmt.bufPrintZ(&fmtbuf, "<goto {s}> at line {d} jumps into the scope of local '{s}'", .{
            std.mem.span(@as([*:0]const u8, @ptrCast(@as([*c]u8, @ptrCast(@alignCast(&gt.*.name.*.contents[@as(usize, @intCast(0))])))))),
            gt.*.line,
            std.mem.span(@as([*:0]const u8, @ptrCast(varname))),
        }) catch "<goto ?> at line ? jumps into the scope of local '?'";
        break :blk lua_pushstring(ls.*.L, m);
    };
    luaK_semerror(ls, msg);
}
pub fn solvegoto(arg_ls: [*c]LexState, arg_g: c_int, arg_label: [*c]Labeldesc) callconv(.c) void {
    var ls = arg_ls;
    _ = &ls;
    var g = arg_g;
    _ = &g;
    ...
    var i: c_int = undefined;
    _ = &i;
    var gl: [*c]Labellist = &ls.*.dyd.*.gt;
    _ = &gl;
    var gt: [*c]Labeldesc = &(blk: {
        const tmp = g;
        if (tmp >= 0) break :blk gl.*.arr + @as(usize, @intCast(tmp)) else break :blk gl.*.arr - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
    }).*;
    _ = &gt;
    _ = @as(c_int, 0);
...
pub fn findlabel(arg_ls: [*c]LexState, arg_name: [*c]TString) callconv(.c) [*c]Labeldesc {
    var ls = arg_ls;
    _ = &ls;
    var name = arg_name;
    _ = &name;
    var i: c_int = undefined;
    _ = &i;
    var dyd: [*c]Dyndata = ls.*.dyd;
    _ = &dyd;
    {
        i = ls.*.fs.*.firstlabel;
        while (i < dyd.*.label.n) : (i += 1) {
            var lb: [*c]Labeldesc = &(blk: {
                const tmp = i;
                if (tmp >= 0) break :blk dyd.*.label.arr + @as(usize, @intCast(tmp)) else break :blk dyd.*.label.arr - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
            }).*;
            _ = &lb;
...
pub fn newlabelentry(arg_ls: [*c]LexState, arg_l: [*c]Labellist, arg_name: [*c]TString, arg_line: c_int, arg_pc: c_int) callconv(.c) c_int {
    var ls = arg_ls;
    _ = &ls;
    var l = arg_l;
    _ = &l;
    ...
    (blk: {
        const tmp = n;
        if (tmp >= 0) break :blk l.*.arr + @as(usize, @intCast(tmp)) else break :blk l.*.arr - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
    }).*.name = name;
    (blk: {
        const tmp = n;
        if (tmp >= 0) break :blk l.*.arr + @as(usize, @intCast(tmp)) else break :blk l.*.arr - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
...
pub fn newgotoentry(arg_ls: [*c]LexState, arg_name: [*c]TString, arg_line: c_int, arg_pc: c_int) callconv(.c) c_int {
    var ls = arg_ls;
    _ = &ls;
    var name = arg_name;
    _ = &name;
...
pub fn solvegotos(arg_ls: [*c]LexState, arg_lb: [*c]Labeldesc) callconv(.c) c_int {
    var ls = arg_ls;
    _ = &ls;
    var lb = arg_lb;
    _ = &lb;
    var gl: [*c]Labellist = &ls.*.dyd.*.gt;
    _ = &gl;
    var i: c_int = ls.*.fs.*.bl.*.firstgoto;
    _ = &i;
    var needsclose: c_int = 0;
    _ = &needsclose;
    while (i < gl.*.n) {
        if ((blk: {
            const tmp = i;
            ...
            needsclose |= @as(c_int, @bitCast(@as(c_uint, (blk: {
                const tmp = i;
                if (tmp >= 0) break :blk gl.*.arr + @as(usize, @intCast(tmp)) else break :blk gl.*.arr - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
            }).*.close)));
            solvegoto(ls, i, lb);
...
pub fn createlabel(arg_ls: [*c]LexState, arg_name: [*c]TString, arg_line: c_int, arg_last: c_int) callconv(.c) c_int {
    var ls = arg_ls;
    _ = &ls;
    var name = arg_name;
    _ = &name;
    ...
    var fs: [*c]FuncState = ls.*.fs;
    _ = &fs;
    var ll: [*c]Labellist = &ls.*.dyd.*.label;
    _ = &ll;
    var l: c_int = newlabelentry(ls, ll, name, line, luaK_getlabel(fs));
    _ = &l;
    if (last != 0) {
        (blk: {
            const tmp = l;
            if (tmp >= 0) break :blk ll.*.arr + @as(usize, @intCast(tmp)) else break :blk ll.*.arr - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
        }).*.nactvar = fs.*.bl.*.nactvar;
    }
    if (solvegotos(ls, &(blk: {
        const tmp = l;
        if (tmp >= 0) break :blk ll.*.arr + @as(usize, @intCast(tmp)) else break :blk ll.*.arr - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
    }).*) != 0) {
        _ = luaK_codeABCk(fs, @as(c_uint, @bitCast(OP_CLOSE)), luaY_nvarstack(fs), @as(c_int, 0), @as(c_int, 0), @as(c_int, 0));
        return 1;
    }
    return 0;
}
pub fn movegotosout(arg_fs: [*c]FuncState, arg_bl: [*c]BlockCnt) callconv(.c) void {
    var fs = arg_fs;
    _ = &fs;
    var bl = arg_bl;
    _ = &bl;
    var i: c_int = undefined;
    _ = &i;
    var gl: [*c]Labellist = &fs.*.ls.*.dyd.*.gt;
    _ = &gl;
    {
        i = bl.*.firstgoto;
        while (i < gl.*.n) : (i += 1) {
            ...
            _ = &gt;
            if (reglevel(fs, @as(c_int, @bitCast(@as(c_uint, gt.*.nactvar)))) > reglevel(fs, @as(c_int, @bitCast(@as(c_uint, bl.*.nactvar))))) {
                gt.*.close |= @as(lu_byte, @bitCast(@as(i8, @truncate(@as(c_int, @bitCast(@as(c_uint, bl.*.upval)))))));
            }
            gt.*.nactvar = bl.*.nactvar;
        }
    }
}
pub fn enterblock(arg_fs: [*c]FuncState, arg_bl: [*c]BlockCnt, arg_isloop: lu_byte) callconv(.c) void {
    var fs = arg_fs;
    _ = &fs;
    var bl = arg_bl;
    _ = &bl;
    var isloop = arg_isloop;
    _ = &isloop;
    bl.*.isloop = isloop;
    ...
    bl.*.upval = 0;
    bl.*.insidetbc = @as(lu_byte, @intFromBool((fs.*.bl != @as([*c]struct_BlockCnt, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) and (@as(c_int, @bitCast(@as(c_uint, fs.*.bl.*.insidetbc))) != 0)));
    bl.*.previous = fs.*.bl;
    fs.*.bl = bl;
    _ = @as(c_int, 0);
}
pub fn undefgoto(arg_ls: [*c]LexState, arg_gt: [*c]Labeldesc) callconv(.c) noreturn {
    var ls = arg_ls;
    _ = &ls;
    var gt = arg_gt;
    _ = &gt;
    ...
    if (gt.*.name == luaS_newlstr(ls.*.L, "break", (@sizeOf([6]u8) / @sizeOf(u8)) -% @as(c_ulong, @bitCast(@as(c_long, @as(c_int, 1)))))) {
        ...
        msg = blk: {
            const m = std.fmt.bufPrintZ(&fmtbuf, "no visible label '{s}' for <goto> at line {d}", .{
                std.mem.span(@as([*:0]const u8, @ptrCast(@as([*c]u8, @ptrCast(@alignCast(&gt.*.name.*.contents[@as(usize, @intCast(0))])))))),
                gt.*.line,
            }) catch "no visible label '?' for <goto> at line ?";
            break :blk lua_pushstring(ls.*.L, m);
        };
    }
    luaK_semerror(ls, msg);
}
pub fn leaveblock(arg_fs: [*c]FuncState) callconv(.c) void {
    var fs = arg_fs;
    _ = &fs;
    var bl: [*c]BlockCnt = fs.*.bl;
    _ = &bl;
    var ls: [*c]LexState = fs.*.ls;
    _ = &ls;
    var hasclose: c_int = 0;
    _ = &hasclose;
    var stklevel: c_int = reglevel(fs, @as(c_int, @bitCast(@as(c_uint, bl.*.nactvar))));
    _ = &stklevel;
    ...
    fs.*.freereg = @as(lu_byte, @bitCast(@as(i8, @truncate(stklevel))));
    ...
    if (bl.*.previous != null) {
        ...
        if (bl.*.firstgoto < ls.*.dyd.*.gt.n) {
            undefgoto(ls, &(blk: {
                const tmp = bl.*.firstgoto;
                if (tmp >= 0) break :blk ls.*.dyd.*.gt.arr + @as(usize, @intCast(tmp)) else break :blk ls.*.dyd.*.gt.arr - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
            }).*);
        }
    }
}
pub fn addprototype(arg_ls: [*c]LexState) callconv(.c) [*c]Proto {
    var ls = arg_ls;
    _ = &ls;
    var clp: [*c]Proto = undefined;
    _ = &clp;
    ...
    if (fs.*.np >= f.*.sizep) {
        ...
        while (oldsize < f.*.sizep) {
            (blk: {
                ...
                if (tmp >= 0) break :blk f.*.p + @as(usize, @intCast(tmp)) else break :blk f.*.p - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
            }).* = null;
        }
    }
    (blk: {
    ...
    _ = if (((@as(c_int, @bitCast(@as(c_uint, f.*.marked))) & (@as(c_int, 1) << @intCast(@as(c_int, 5)))) != 0) and ((@as(c_int, @bitCast(@as(c_uint, clp.*.marked))) & ((@as(c_int, 1) << @intCast(@as(c_int, 3))) | (@as(c_int, 1) << @intCast(@as(c_int, 4))))) != 0)) luaC_barrier_(L, blk: {
        _ = @as(c_int, 0);
        break :blk &@as([*c]union_GCUnion, @ptrCast(@alignCast(f))).*.gc;
    }, blk: {
        _ = @as(c_int, 0);
        break :blk &@as([*c]union_GCUnion, @ptrCast(@alignCast(clp))).*.gc;
    }) else @as(c_int, 0);
    return clp;
}
pub fn codeclosure(arg_ls: [*c]LexState, arg_v: [*c]expdesc) callconv(.c) void {
    var ls = arg_ls;
    _ = &ls;
    var v = arg_v;
    _ = &v;
    var fs: [*c]FuncState = ls.*.fs.*.prev;
    _ = &fs;
    init_exp(v, @as(c_uint, @bitCast(VRELOC)), luaK_codeABx(fs, @as(c_uint, @bitCast(OP_CLOSURE)), @as(c_int, 0), @as(c_uint, @bitCast(fs.*.np - @as(c_int, 1)))));
    luaK_exp2nextreg(fs, v);
}
pub fn open_func(arg_ls: [*c]LexState, arg_fs: [*c]FuncState, arg_bl: [*c]BlockCnt) callconv(.c) void {
    var ls = arg_ls;
    _ = &ls;
    var fs = arg_fs;
    _ = &fs;
    ...
    fs.*.bl = null;
    f.*.source = ls.*.source;
    _ = if (((@as(c_int, @bitCast(@as(c_uint, f.*.marked))) & (@as(c_int, 1) << @intCast(@as(c_int, 5)))) != 0) and ((@as(c_int, @bitCast(@as(c_uint, f.*.source.*.marked))) & ((@as(c_int, 1) << @intCast(@as(c_int, 3))) | (@as(c_int, 1) << @intCast(@as(c_int, 4))))) != 0)) luaC_barrier_(ls.*.L, blk: {
        _ = @as(c_int, 0);
        break :blk &@as([*c]union_GCUnion, @ptrCast(@alignCast(f))).*.gc;
    }, blk: {
        _ = @as(c_int, 0);
        break :blk &@as([*c]union_GCUnion, @ptrCast(@alignCast(f.*.source))).*.gc;
    }) else @as(c_int, 0);
    f.*.maxstacksize = 2;
    enterblock(fs, bl, @as(lu_byte, @bitCast(@as(i8, @truncate(@as(c_int, 0))))));
}
pub fn close_func(arg_ls: [*c]LexState) callconv(.c) void {
    var ls = arg_ls;
    _ = &ls;
    var L: [*c]lua_State = ls.*.L;
    _ = &L;
    ...
    luaK_finish(fs);
    _ = blk: {
        const tmp = @as([*c]Instruction, @ptrCast(@alignCast(luaM_shrinkvector_(L, @as(?*anyopaque, @ptrCast(f.*.code)), &f.*.sizecode, fs.*.pc, @as(c_int, @bitCast(@as(c_uint, @truncate(@sizeOf(Instruction)))))))));
        f.*.code = tmp;
        break :blk tmp;
    };
    _ = blk: {
        const tmp = @as([*c]ls_byte, @ptrCast(@alignCast(luaM_shrinkvector_(L, @as(?*anyopaque, @ptrCast(f.*.lineinfo)), &f.*.sizelineinfo, fs.*.pc, @as(c_int, @bitCast(@as(c_uint, @truncate(@sizeOf(ls_byte)))))))));
        f.*.lineinfo = tmp;
        break :blk tmp;
    };
    _ = blk: {
        const tmp = @as([*c]AbsLineInfo, @ptrCast(@alignCast(luaM_shrinkvector_(L, @as(?*anyopaque, @ptrCast(f.*.abslineinfo)), &f.*.sizeabslineinfo, fs.*.nabslineinfo, @as(c_int, @bitCast(@as(c_uint, @truncate(@sizeOf(AbsLineInfo)))))))));
        f.*.abslineinfo = tmp;
        break :blk tmp;
    };
    _ = blk: {
        const tmp = @as([*c]TValue, @ptrCast(@alignCast(luaM_shrinkvector_(L, @as(?*anyopaque, @ptrCast(f.*.k)), &f.*.sizek, fs.*.nk, @as(c_int, @bitCast(@as(c_uint, @truncate(@sizeOf(TValue)))))))));
        f.*.k = tmp;
        break :blk tmp;
    };
    _ = blk: {
        const tmp = @as([*c][*c]Proto, @ptrCast(@alignCast(luaM_shrinkvector_(L, @as(?*anyopaque, @ptrCast(f.*.p)), &f.*.sizep, fs.*.np, @as(c_int, @bitCast(@as(c_uint, @truncate(@sizeOf([*c]Proto)))))))));
        f.*.p = tmp;
        break :blk tmp;
    };
    _ = blk: {
        const tmp = @as([*c]LocVar, @ptrCast(@alignCast(luaM_shrinkvector_(L, @as(?*anyopaque, @ptrCast(f.*.locvars)), &f.*.sizelocvars, @as(c_int, @bitCast(@as(c_int, fs.*.ndebugvars))), @as(c_int, @bitCast(@as(c_uint, @truncate(@sizeOf(LocVar)))))))));
        f.*.locvars = tmp;
        break :blk tmp;
    };
    _ = blk: {
        const tmp = @as([*c]Upvaldesc, @ptrCast(@alignCast(luaM_shrinkvector_(L, @as(?*anyopaque, @ptrCast(f.*.upvalues)), &f.*.sizeupvalues, @as(c_int, @bitCast(@as(c_uint, fs.*.nups))), @as(c_int, @bitCast(@as(c_uint, @truncate(@sizeOf(Upvaldesc)))))))));
        f.*.upvalues = tmp;
        break :blk tmp;
    };
    ls.*.fs = fs.*.prev;
...
pub fn block_follow(arg_ls: [*c]LexState, arg_withuntil: c_int) callconv(.c) c_int {
    var ls = arg_ls;
    _ = &ls;
    var withuntil = arg_withuntil;
    _ = &withuntil;
    while (true) {
        switch (ls.*.t.token) {
            @as(c_int, 259), @as(c_int, 260), @as(c_int, 261), @as(c_int, 288) => return 1,
            @as(c_int, 276) => return withuntil,
            else => return 0,
        }
        break;
    }
...
pub fn statlist(arg_ls: [*c]LexState) callconv(.c) void {
    var ls = arg_ls;
    _ = &ls;
    while (!(block_follow(ls, @as(c_int, 1)) != 0)) {
...
pub fn fieldsel(arg_ls: [*c]LexState, arg_v: [*c]expdesc) callconv(.c) void {
    var ls = arg_ls;
    _ = &ls;
    var v = arg_v;
    _ = &v;
...
pub fn yindex(arg_ls: [*c]LexState, arg_v: [*c]expdesc) callconv(.c) void {
    var ls = arg_ls;
    _ = &ls;
    var v = arg_v;
    _ = &v;
...
pub const struct_ConsControl = extern struct {
    v: expdesc = @import("std").mem.zeroes(expdesc),
    t: [*c]expdesc = @import("std").mem.zeroes([*c]expdesc),
    nh: c_int = @import("std").mem.zeroes(c_int),
    na: c_int = @import("std").mem.zeroes(c_int),
    tostore: c_int = @import("std").mem.zeroes(c_int),
};
pub const ConsControl = struct_ConsControl;
pub fn recfield(arg_ls: [*c]LexState, arg_cc: [*c]ConsControl) callconv(.c) void {
    var ls = arg_ls;
    _ = &ls;
    var cc = arg_cc;
    _ = &cc;
    var fs: [*c]FuncState = ls.*.fs;
    _ = &fs;
    var reg: c_int = @as(c_int, @bitCast(@as(c_uint, ls.*.fs.*.freereg)));
    _ = &reg;
    ...
    if (ls.*.t.token == TK_NAME) {
        checklimit(fs, cc.*.nh, @as(c_int, 2147483647), "items in a constructor");
        codename(ls, &key);
    } else {
        yindex(ls, &key);
    }
    ...
    fs.*.freereg = @as(lu_byte, @bitCast(@as(i8, @truncate(reg))));
}
pub fn closelistfield(arg_fs: [*c]FuncState, arg_cc: [*c]ConsControl) callconv(.c) void {
    var fs = arg_fs;
    _ = &fs;
    var cc = arg_cc;
    _ = &cc;
    if (cc.*.v.k == @as(c_uint, @bitCast(VVOID))) return;
    luaK_exp2nextreg(fs, &cc.*.v);
    cc.*.v.k = @as(c_uint, @bitCast(VVOID));
    if (cc.*.tostore == @as(c_int, 50)) {
        luaK_setlist(fs, cc.*.t.*.u.info, cc.*.na, cc.*.tostore);
        cc.*.na += cc.*.tostore;
        cc.*.tostore = 0;
    }
}
pub fn lastlistfield(arg_fs: [*c]FuncState, arg_cc: [*c]ConsControl) callconv(.c) void {
    var fs = arg_fs;
    _ = &fs;
    var cc = arg_cc;
    _ = &cc;
    if (cc.*.tostore == @as(c_int, 0)) return;
    if ((cc.*.v.k == @as(c_uint, @bitCast(VCALL))) or (cc.*.v.k == @as(c_uint, @bitCast(VVARARG)))) {
        luaK_setreturns(fs, &cc.*.v, -@as(c_int, 1));
        luaK_setlist(fs, cc.*.t.*.u.info, cc.*.na, -@as(c_int, 1));
        cc.*.na -= 1;
    } else {
        if (cc.*.v.k != @as(c_uint, @bitCast(VVOID))) {
            luaK_exp2nextreg(fs, &cc.*.v);
        }
        luaK_setlist(fs, cc.*.t.*.u.info, cc.*.na, cc.*.tostore);
    }
...
pub fn listfield(arg_ls: [*c]LexState, arg_cc: [*c]ConsControl) callconv(.c) void {
    var ls = arg_ls;
    _ = &ls;
    var cc = arg_cc;
    _ = &cc;
...
pub fn field(arg_ls: [*c]LexState, arg_cc: [*c]ConsControl) callconv(.c) void {
    var ls = arg_ls;
    _ = &ls;
    var cc = arg_cc;
    _ = &cc;
...
pub fn constructor(arg_ls: [*c]LexState, arg_t: [*c]expdesc) callconv(.c) void {
    var ls = arg_ls;
    _ = &ls;
    var t = arg_t;
    _ = &t;
    ...
    _ = &line;
    var pc: c_int = luaK_codeABCk(fs, @as(c_uint, @bitCast(OP_NEWTABLE)), @as(c_int, 0), @as(c_int, 0), @as(c_int, 0), @as(c_int, 0));
    _ = &pc;
    var cc: ConsControl = undefined;
    _ = &cc;
    _ = luaK_code(fs, @as(Instruction, @bitCast(@as(c_int, 0))));
    cc.na = blk: {
        const tmp = blk_1: {
    ...
    cc.t = t;
    init_exp(t, @as(c_uint, @bitCast(VNONRELOC)), @as(c_int, @bitCast(@as(c_uint, fs.*.freereg))));
    luaK_reserveregs(fs, @as(c_int, 1));
    init_exp(&cc.v, @as(c_uint, @bitCast(VVOID)), @as(c_int, 0));
    checknext(ls, @as(c_int, '{'));
    while (true) {
        _ = @as(c_int, 0);
        if (ls.*.t.token == @as(c_int, '}')) break;
        closelistfield(fs, &cc);
        field(ls, &cc);
        if (!((testnext(ls, @as(c_int, ',')) != 0) or (testnext(ls, @as(c_int, ';')) != 0))) break;
    }
    check_match(ls, @as(c_int, '}'), @as(c_int, '{'), line);
    lastlistfield(fs, &cc);
    luaK_settablesize(fs, pc, t.*.u.info, cc.na, cc.nh);
}
pub fn setvararg(arg_fs: [*c]FuncState, arg_nparams: c_int) callconv(.c) void {
    var fs = arg_fs;
    _ = &fs;
    var nparams = arg_nparams;
    _ = &nparams;
    fs.*.f.*.is_vararg = 1;
    _ = luaK_codeABCk(fs, @as(c_uint, @bitCast(OP_VARARGPREP)), nparams, @as(c_int, 0), @as(c_int, 0), @as(c_int, 0));
}
pub fn parlist(arg_ls: [*c]LexState) callconv(.c) void {
    var ls = arg_ls;
    _ = &ls;
    var fs: [*c]FuncState = ls.*.fs;
    _ = &fs;
    ...
    if (ls.*.t.token != @as(c_int, ')')) {
        while (true) {
            ...
            if (!(!(isvararg != 0) and (testnext(ls, @as(c_int, ',')) != 0))) break;
        }
    }
    adjustlocalvars(ls, nparams);
    f.*.numparams = fs.*.nactvar;
    if (isvararg != 0) {
        setvararg(fs, @as(c_int, @bitCast(@as(c_uint, f.*.numparams))));
    }
    luaK_reserveregs(fs, @as(c_int, @bitCast(@as(c_uint, fs.*.nactvar))));
}
pub fn body(arg_ls: [*c]LexState, arg_e: [*c]expdesc, arg_ismethod: c_int, arg_line: c_int) callconv(.c) void {
    var ls = arg_ls;
    _ = &ls;
    var e = arg_e;
    _ = &e;
    ...
    var line = arg_line;
    _ = &line;
    var new_fs: FuncState = undefined;
    _ = &new_fs;
    var bl: BlockCnt = undefined;
    _ = &bl;
...
pub fn explist(arg_ls: [*c]LexState, arg_v: [*c]expdesc) callconv(.c) c_int {
    var ls = arg_ls;
    _ = &ls;
    var v = arg_v;
    _ = &v;
...
pub fn funcargs(arg_ls: [*c]LexState, arg_f: [*c]expdesc, arg_line: c_int) callconv(.c) void {
    var ls = arg_ls;
    _ = &ls;
    var f = arg_f;
    _ = &f;
            ...
            @as(c_int, 40) => {
                {
                    luaX_next(ls);
                    if (ls.*.t.token == @as(c_int, ')')) {
                        args.k = @as(c_uint, @bitCast(VVOID));
                    } else {
                        _ = explist(ls, &args);
                        if ((args.k == @as(c_uint, @bitCast(VCALL))) or (args.k == @as(c_uint, @bitCast(VVARARG)))) {
                            luaK_setreturns(fs, &args, -@as(c_int, 1));
                        }
                    }
                    check_match(ls, @as(c_int, ')'), @as(c_int, '('), line);
                    break;
    ...
    if ((args.k == @as(c_uint, @bitCast(VCALL))) or (args.k == @as(c_uint, @bitCast(VVARARG)))) {
        ...
        if (args.k != @as(c_uint, @bitCast(VVOID))) {
            luaK_exp2nextreg(fs, &args);
        }
        nparams = @as(c_int, @bitCast(@as(c_uint, fs.*.freereg))) - (base + @as(c_int, 1));
    }
    init_exp(f, @as(c_uint, @bitCast(VCALL)), luaK_codeABCk(fs, @as(c_uint, @bitCast(OP_CALL)), base, nparams + @as(c_int, 1), @as(c_int, 2), @as(c_int, 0)));
    luaK_fixline(fs, line);
    fs.*.freereg = @as(lu_byte, @bitCast(@as(i8, @truncate(base + @as(c_int, 1)))));
}
pub fn primaryexp(arg_ls: [*c]LexState, arg_v: [*c]expdesc) callconv(.c) void {
    var ls = arg_ls;
    _ = &ls;
    var v = arg_v;
    _ = &v;
...
pub fn suffixedexp(arg_ls: [*c]LexState, arg_v: [*c]expdesc) callconv(.c) void {
    var ls = arg_ls;
    _ = &ls;
    var v = arg_v;
    _ = &v;
    ...
    while (true) {
        while (true) {
            switch (ls.*.t.token) {
                ...
                @as(c_int, 40), @as(c_int, 292), @as(c_int, 123) => {
                    {
                        luaK_exp2nextreg(fs, v);
                        funcargs(ls, v, line);
                        break;
...
pub fn simpleexp(arg_ls: [*c]LexState, arg_v: [*c]expdesc) callconv(.c) void {
    var ls = arg_ls;
    _ = &ls;
    var v = arg_v;
    _ = &v;
    while (true) {
        switch (ls.*.t.token) {
            @as(c_int, 289) => {
                {
                    init_exp(v, @as(c_uint, @bitCast(VKFLT)), @as(c_int, 0));
            ...
            @as(c_int, 290) => {
                {
                    init_exp(v, @as(c_uint, @bitCast(VKINT)), @as(c_int, 0));
            ...
            @as(c_int, 269) => {
                {
                    init_exp(v, @as(c_uint, @bitCast(VNIL)), @as(c_int, 0));
                    break;
                }
            },
            @as(c_int, 275) => {
                {
                    init_exp(v, @as(c_uint, @bitCast(VTRUE)), @as(c_int, 0));
                    break;
                }
            },
            @as(c_int, 262) => {
                {
                    init_exp(v, @as(c_uint, @bitCast(VFALSE)), @as(c_int, 0));
                    break;
                }
            },
            @as(c_int, 280) => {
                {
                    ...
                    init_exp(v, @as(c_uint, @bitCast(VVARARG)), luaK_codeABCk(fs, @as(c_uint, @bitCast(OP_VARARG)), @as(c_int, 0), @as(c_int, 0), @as(c_int, 1), @as(c_int, 0)));
                    break;
                }
            },
            @as(c_int, 123) => {
            ...
            @as(c_int, 264) => {
                {
                    luaX_next(ls);
                    body(ls, v, @as(c_int, 0), ls.*.linenumber);
                    return;
                }
            },
            else => {
...
pub fn getunopr(arg_op: c_int) callconv(.c) UnOpr {
    var op = arg_op;
    _ = &op;
    while (true) {
        switch (op) {
            @as(c_int, 270) => return @as(c_uint, @bitCast(OPR_NOT)),
            @as(c_int, 45) => return @as(c_uint, @bitCast(OPR_MINUS)),
            @as(c_int, 126) => return @as(c_uint, @bitCast(OPR_BNOT)),
            @as(c_int, 35) => return @as(c_uint, @bitCast(OPR_LEN)),
            else => return @as(c_uint, @bitCast(OPR_NOUNOPR)),
        }
        break;
    }
    return @import("std").mem.zeroes(UnOpr);
}
pub fn getbinopr(arg_op: c_int) callconv(.c) BinOpr {
    var op = arg_op;
    _ = &op;
    while (true) {
        switch (op) {
            @as(c_int, 43) => return @as(c_uint, @bitCast(OPR_ADD)),
            @as(c_int, 45) => return @as(c_uint, @bitCast(OPR_SUB)),
            @as(c_int, 42) => return @as(c_uint, @bitCast(OPR_MUL)),
            @as(c_int, 37) => return @as(c_uint, @bitCast(OPR_MOD)),
            @as(c_int, 94) => return @as(c_uint, @bitCast(OPR_POW)),
            @as(c_int, 47) => return @as(c_uint, @bitCast(OPR_DIV)),
            @as(c_int, 278) => return @as(c_uint, @bitCast(OPR_IDIV)),
            @as(c_int, 38) => return @as(c_uint, @bitCast(OPR_BAND)),
            @as(c_int, 124) => return @as(c_uint, @bitCast(OPR_BOR)),
            @as(c_int, 126) => return @as(c_uint, @bitCast(OPR_BXOR)),
            @as(c_int, 285) => return @as(c_uint, @bitCast(OPR_SHL)),
            @as(c_int, 286) => return @as(c_uint, @bitCast(OPR_SHR)),
            @as(c_int, 279) => return @as(c_uint, @bitCast(OPR_CONCAT)),
            @as(c_int, 284) => return @as(c_uint, @bitCast(OPR_NE)),
            @as(c_int, 281) => return @as(c_uint, @bitCast(OPR_EQ)),
            @as(c_int, 60) => return @as(c_uint, @bitCast(OPR_LT)),
            @as(c_int, 283) => return @as(c_uint, @bitCast(OPR_LE)),
            @as(c_int, 62) => return @as(c_uint, @bitCast(OPR_GT)),
            @as(c_int, 282) => return @as(c_uint, @bitCast(OPR_GE)),
            @as(c_int, 256) => return @as(c_uint, @bitCast(OPR_AND)),
            @as(c_int, 271) => return @as(c_uint, @bitCast(OPR_OR)),
            else => return @as(c_uint, @bitCast(OPR_NOBINOPR)),
        }
        break;
    }
    return @import("std").mem.zeroes(BinOpr);
}
const struct_unnamed_19 = extern struct {
    left: lu_byte = @import("std").mem.zeroes(lu_byte),
    right: lu_byte = @import("std").mem.zeroes(lu_byte),
};
pub const priority: [21]struct_unnamed_19 = [21]struct_unnamed_19{
    struct_unnamed_19{
        .left = @as(lu_byte, @bitCast(@as(i8, @truncate(@as(c_int, 10))))),
        .right = @as(lu_byte, @bitCast(@as(i8, @truncate(@as(c_int, 10))))),
    },
    struct_unnamed_19{
        .left = @as(lu_byte, @bitCast(@as(i8, @truncate(@as(c_int, 10))))),
        .right = @as(lu_byte, @bitCast(@as(i8, @truncate(@as(c_int, 10))))),
    ...
    struct_unnamed_19{
        .left = @as(lu_byte, @bitCast(@as(i8, @truncate(@as(c_int, 11))))),
        .right = @as(lu_byte, @bitCast(@as(i8, @truncate(@as(c_int, 11))))),
    },
    struct_unnamed_19{
        .left = @as(lu_byte, @bitCast(@as(i8, @truncate(@as(c_int, 11))))),
        .right = @as(lu_byte, @bitCast(@as(i8, @truncate(@as(c_int, 11))))),
    ...
    struct_unnamed_19{
        .left = @as(lu_byte, @bitCast(@as(i8, @truncate(@as(c_int, 14))))),
        .right = @as(lu_byte, @bitCast(@as(i8, @truncate(@as(c_int, 13))))),
    },
    struct_unnamed_19{
        .left = @as(lu_byte, @bitCast(@as(i8, @truncate(@as(c_int, 11))))),
        .right = @as(lu_byte, @bitCast(@as(i8, @truncate(@as(c_int, 11))))),
    ...
    struct_unnamed_19{
        .left = @as(lu_byte, @bitCast(@as(i8, @truncate(@as(c_int, 6))))),
        .right = @as(lu_byte, @bitCast(@as(i8, @truncate(@as(c_int, 6))))),
    },
    struct_unnamed_19{
        .left = @as(lu_byte, @bitCast(@as(i8, @truncate(@as(c_int, 4))))),
        .right = @as(lu_byte, @bitCast(@as(i8, @truncate(@as(c_int, 4))))),
    },
    struct_unnamed_19{
        .left = @as(lu_byte, @bitCast(@as(i8, @truncate(@as(c_int, 5))))),
        .right = @as(lu_byte, @bitCast(@as(i8, @truncate(@as(c_int, 5))))),
    },
    struct_unnamed_19{
        .left = @as(lu_byte, @bitCast(@as(i8, @truncate(@as(c_int, 7))))),
        .right = @as(lu_byte, @bitCast(@as(i8, @truncate(@as(c_int, 7))))),
    },
    struct_unnamed_19{
        .left = @as(lu_byte, @bitCast(@as(i8, @truncate(@as(c_int, 7))))),
        .right = @as(lu_byte, @bitCast(@as(i8, @truncate(@as(c_int, 7))))),
    ...
    struct_unnamed_19{
        .left = @as(lu_byte, @bitCast(@as(i8, @truncate(@as(c_int, 9))))),
        .right = @as(lu_byte, @bitCast(@as(i8, @truncate(@as(c_int, 8))))),
    },
    struct_unnamed_19{
        .left = @as(lu_byte, @bitCast(@as(i8, @truncate(@as(c_int, 3))))),
        .right = @as(lu_byte, @bitCast(@as(i8, @truncate(@as(c_int, 3))))),
    },
    struct_unnamed_19{
        .left = @as(lu_byte, @bitCast(@as(i8, @truncate(@as(c_int, 3))))),
        .right = @as(lu_byte, @bitCast(@as(i8, @truncate(@as(c_int, 3))))),
    ...
    struct_unnamed_19{
        .left = @as(lu_byte, @bitCast(@as(i8, @truncate(@as(c_int, 2))))),
        .right = @as(lu_byte, @bitCast(@as(i8, @truncate(@as(c_int, 2))))),
    },
    struct_unnamed_19{
        .left = @as(lu_byte, @bitCast(@as(i8, @truncate(@as(c_int, 1))))),
        .right = @as(lu_byte, @bitCast(@as(i8, @truncate(@as(c_int, 1))))),
    },
};
pub fn subexpr(arg_ls: [*c]LexState, arg_v: [*c]expdesc, arg_limit: c_int) callconv(.c) BinOpr {
    var ls = arg_ls;
    _ = &ls;
    var v = arg_v;
    _ = &v;
    ...
    _ = &op;
    var uop: UnOpr = undefined;
    _ = &uop;
    luaE_incCstack(ls.*.L);
    uop = getunopr(ls.*.t.token);
    if (uop != @as(c_uint, @bitCast(OPR_NOUNOPR))) {
        var line: c_int = ls.*.linenumber;
        _ = &line;
        luaX_next(ls);
        _ = subexpr(ls, v, @as(c_int, 12));
        luaK_prefix(ls.*.fs, uop, v, line);
    } else {
        simpleexp(ls, v);
    }
    op = getbinopr(ls.*.t.token);
    while ((op != @as(c_uint, @bitCast(OPR_NOBINOPR))) and (@as(c_int, @bitCast(@as(c_uint, priority[op].left))) > limit)) {
        ...
        nextop = subexpr(ls, &v2, @as(c_int, @bitCast(@as(c_uint, priority[op].right))));
        luaK_posfix(ls.*.fs, op, v, &v2, line);
        op = nextop;
    }
    _ = blk: {
        const ref = &ls.*.L.*.nCcalls;
        const tmp = ref.*;
        ref.* -%= 1;
        break :blk tmp;
    };
...
pub fn block(arg_ls: [*c]LexState) callconv(.c) void {
    var ls = arg_ls;
    _ = &ls;
    var fs: [*c]FuncState = ls.*.fs;
    _ = &fs;
...
pub const struct_LHS_assign = extern struct {
    prev: [*c]struct_LHS_assign = @import("std").mem.zeroes([*c]struct_LHS_assign),
    v: expdesc = @import("std").mem.zeroes(expdesc),
};
pub fn check_conflict(arg_ls: [*c]LexState, arg_lh: [*c]struct_LHS_assign, arg_v: [*c]expdesc) callconv(.c) void {
    var ls = arg_ls;
    _ = &ls;
    var lh = arg_lh;
    _ = &lh;
    ...
    var fs: [*c]FuncState = ls.*.fs;
    _ = &fs;
    var extra: c_int = @as(c_int, @bitCast(@as(c_uint, fs.*.freereg)));
    _ = &extra;
    var conflict: c_int = 0;
    _ = &conflict;
    while (lh != null) : (lh = lh.*.prev) {
        if ((@as(c_uint, @bitCast(VINDEXED)) <= lh.*.v.k) and (lh.*.v.k <= @as(c_uint, @bitCast(VINDEXSTR)))) {
            if (lh.*.v.k == @as(c_uint, @bitCast(VINDEXUP))) {
                if ((v.*.k == @as(c_uint, @bitCast(VUPVAL))) and (@as(c_int, @bitCast(@as(c_uint, lh.*.v.u.ind.t))) == v.*.u.info)) {
                    conflict = 1;
                    lh.*.v.k = @as(c_uint, @bitCast(VINDEXSTR));
                    lh.*.v.u.ind.t = @as(lu_byte, @bitCast(@as(i8, @truncate(extra))));
                }
            } else {
                if ((v.*.k == @as(c_uint, @bitCast(VLOCAL))) and (@as(c_int, @bitCast(@as(c_uint, lh.*.v.u.ind.t))) == @as(c_int, @bitCast(@as(c_uint, v.*.u.@"var".ridx))))) {
                    conflict = 1;
                    lh.*.v.u.ind.t = @as(lu_byte, @bitCast(@as(i8, @truncate(extra))));
                }
                if (((lh.*.v.k == @as(c_uint, @bitCast(VINDEXED))) and (v.*.k == @as(c_uint, @bitCast(VLOCAL)))) and (@as(c_int, @bitCast(@as(c_int, lh.*.v.u.ind.idx))) == @as(c_int, @bitCast(@as(c_uint, v.*.u.@"var".ridx))))) {
                    conflict = 1;
                    lh.*.v.u.ind.idx = @as(c_short, @bitCast(@as(c_short, @truncate(extra))));
                }
            }
        }
    }
    if (conflict != 0) {
        if (v.*.k == @as(c_uint, @bitCast(VLOCAL))) {
            _ = luaK_codeABCk(fs, @as(c_uint, @bitCast(OP_MOVE)), extra, @as(c_int, @bitCast(@as(c_uint, v.*.u.@"var".ridx))), @as(c_int, 0), @as(c_int, 0));
        } else {
            _ = luaK_codeABCk(fs, @as(c_uint, @bitCast(OP_GETUPVAL)), extra, v.*.u.info, @as(c_int, 0), @as(c_int, 0));
        }
        luaK_reserveregs(fs, @as(c_int, 1));
    }
}
pub fn restassign(arg_ls: [*c]LexState, arg_lh: [*c]struct_LHS_assign, arg_nvars: c_int) callconv(.c) void {
    var ls = arg_ls;
    _ = &ls;
    var lh = arg_lh;
    _ = &lh;
    ...
    _ = &e;
    {
        if (!((@as(c_uint, @bitCast(VLOCAL)) <= lh.*.v.k) and (lh.*.v.k <= @as(c_uint, @bitCast(VINDEXSTR))))) {
            luaX_syntaxerror(ls, "syntax error");
        }
    }
    check_readonly(ls, &lh.*.v);
    if (testnext(ls, @as(c_int, ',')) != 0) {
        var nv: struct_LHS_assign = undefined;
        _ = &nv;
        nv.prev = lh;
        suffixedexp(ls, &nv.v);
        if (!((@as(c_uint, @bitCast(VINDEXED)) <= nv.v.k) and (nv.v.k <= @as(c_uint, @bitCast(VINDEXSTR))))) {
            check_conflict(ls, lh, &nv.v);
        }
        luaE_incCstack(ls.*.L);
        restassign(ls, &nv, nvars + @as(c_int, 1));
        _ = blk: {
            const ref = &ls.*.L.*.nCcalls;
            const tmp = ref.*;
            ref.* -%= 1;
    ...
    init_exp(&e, @as(c_uint, @bitCast(VNONRELOC)), @as(c_int, @bitCast(@as(c_uint, ls.*.fs.*.freereg))) - @as(c_int, 1));
    luaK_storevar(ls.*.fs, &lh.*.v, &e);
}
pub fn cond(arg_ls: [*c]LexState) callconv(.c) c_int {
    var ls = arg_ls;
    _ = &ls;
    var v: expdesc = undefined;
    _ = &v;
    expr(ls, &v);
    if (v.k == @as(c_uint, @bitCast(VNIL))) {
        v.k = @as(c_uint, @bitCast(VFALSE));
    }
    luaK_goiftrue(ls.*.fs, &v);
    return v.f;
}
pub fn gotostat(arg_ls: [*c]LexState) callconv(.c) void {
    var ls = arg_ls;
    _ = &ls;
    var fs: [*c]FuncState = ls.*.fs;
    _ = &fs;
    var line: c_int = ls.*.linenumber;
    _ = &line;
    var name: [*c]TString = str_checkname(ls);
    _ = &name;
    var lb: [*c]Labeldesc = findlabel(ls, name);
    _ = &lb;
    if (lb == @as([*c]Labeldesc, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        _ = newgotoentry(ls, name, line, luaK_jump(fs));
    } else {
        var lblevel: c_int = reglevel(fs, @as(c_int, @bitCast(@as(c_uint, lb.*.nactvar))));
        _ = &lblevel;
        if (luaY_nvarstack(fs) > lblevel) {
            _ = luaK_codeABCk(fs, @as(c_uint, @bitCast(OP_CLOSE)), lblevel, @as(c_int, 0), @as(c_int, 0), @as(c_int, 0));
        }
        luaK_patchlist(fs, luaK_jump(fs), lb.*.pc);
    }
}
pub fn breakstat(arg_ls: [*c]LexState) callconv(.c) void {
    var ls = arg_ls;
    _ = &ls;
    var line: c_int = ls.*.linenumber;
    _ = &line;
...
pub fn checkrepeated(arg_ls: [*c]LexState, arg_name: [*c]TString) callconv(.c) void {
    ...
    if (__builtin_expect(@as(c_long, @intFromBool(@intFromBool(lb != @as([*c]Labeldesc, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) != @as(c_int, 0))), @as(c_long, @bitCast(@as(c_long, @as(c_int, 0))))) != 0) {
        ...
        _ = &msg;
        msg = blk: {
            const m = std.fmt.bufPrintZ(&fmtbuf, "label '{s}' already defined on line {d}", .{
                std.mem.span(@as([*:0]const u8, @ptrCast(@as([*c]u8, @ptrCast(@alignCast(&name.*.contents[@as(usize, @intCast(0))])))))),
                lb.*.line,
            }) catch "label '?' already defined on line ?";
            break :blk lua_pushstring(ls.*.L, m);
        };
        luaK_semerror(ls, msg);
    }
}
pub fn labelstat(arg_ls: [*c]LexState, arg_name: [*c]TString, arg_line: c_int) callconv(.c) void {
    var ls = arg_ls;
    _ = &ls;
    var name = arg_name;
    _ = &name;
    ...
    while ((ls.*.t.token == @as(c_int, ';')) or (ls.*.t.token == TK_DBCOLON)) {
        statement(ls);
    }
    checkrepeated(ls, name);
    _ = createlabel(ls, name, line, block_follow(ls, @as(c_int, 0)));
}
pub fn whilestat(arg_ls: [*c]LexState, arg_line: c_int) callconv(.c) void {
    var ls = arg_ls;
    _ = &ls;
    var line = arg_line;
    _ = &line;
    var fs: [*c]FuncState = ls.*.fs;
    _ = &fs;
    var whileinit: c_int = undefined;
    _ = &whileinit;
    var condexit: c_int = undefined;
    _ = &condexit;
    var bl: BlockCnt = undefined;
    _ = &bl;
    luaX_next(ls);
    ...
    condexit = cond(ls);
    enterblock(fs, &bl, @as(lu_byte, @bitCast(@as(i8, @truncate(@as(c_int, 1))))));
    checknext(ls, TK_DO);
    block(ls);
...
pub fn repeatstat(arg_ls: [*c]LexState, arg_line: c_int) callconv(.c) void {
    var ls = arg_ls;
    _ = &ls;
    var line = arg_line;
    _ = &line;
    ...
    var fs: [*c]FuncState = ls.*.fs;
    _ = &fs;
    var repeat_init: c_int = luaK_getlabel(fs);
    _ = &repeat_init;
    var bl1: BlockCnt = undefined;
    _ = &bl1;
    var bl2: BlockCnt = undefined;
    _ = &bl2;
    enterblock(fs, &bl1, @as(lu_byte, @bitCast(@as(i8, @truncate(@as(c_int, 1))))));
    enterblock(fs, &bl2, @as(lu_byte, @bitCast(@as(i8, @truncate(@as(c_int, 0))))));
    luaX_next(ls);
    statlist(ls);
    ...
    leaveblock(fs);
    if (bl2.upval != 0) {
        var exit_1: c_int = luaK_jump(fs);
        _ = &exit_1;
        luaK_patchtohere(fs, condexit);
        _ = luaK_codeABCk(fs, @as(c_uint, @bitCast(OP_CLOSE)), reglevel(fs, @as(c_int, @bitCast(@as(c_uint, bl2.nactvar)))), @as(c_int, 0), @as(c_int, 0), @as(c_int, 0));
        condexit = luaK_jump(fs);
        luaK_patchtohere(fs, exit_1);
    }
    luaK_patchlist(fs, condexit, repeat_init);
    leaveblock(fs);
}
pub fn exp1(arg_ls: [*c]LexState) callconv(.c) void {
    var ls = arg_ls;
    _ = &ls;
    var e: expdesc = undefined;
    _ = &e;
...
pub fn fixforjump(arg_fs: [*c]FuncState, arg_pc: c_int, arg_dest: c_int, arg_back: c_int) callconv(.c) void {
    var fs = arg_fs;
    _ = &fs;
    var pc = arg_pc;
    _ = &pc;
    ...
    _ = blk: {
        const tmp = (jmp.* & ~(~(~@as(Instruction, @bitCast(@as(c_int, 0))) << @intCast((@as(c_int, 8) + @as(c_int, 8)) + @as(c_int, 1))) << @intCast((@as(c_int, 0) + @as(c_int, 7)) + @as(c_int, 8)))) | ((@as(Instruction, @bitCast(offset)) << @intCast((@as(c_int, 0) + @as(c_int, 7)) + @as(c_int, 8))) & (~(~@as(Instruction, @bitCast(@as(c_int, 0))) << @intCast((@as(c_int, 8) + @as(c_int, 8)) + @as(c_int, 1))) << @intCast((@as(c_int, 0) + @as(c_int, 7)) + @as(c_int, 8))));
        jmp.* = tmp;
        break :blk tmp;
    };
}
pub fn forbody(arg_ls: [*c]LexState, arg_base: c_int, arg_line: c_int, arg_nvars: c_int, arg_isgen: c_int) callconv(.c) void {
    var ls = arg_ls;
    _ = &ls;
    var base = arg_base;
    _ = &base;
    ...
    var isgen = arg_isgen;
    _ = &isgen;
    const forprep = struct {
        const static: [2]OpCode = [2]OpCode{
            @as(c_uint, @bitCast(OP_FORPREP)),
            @as(c_uint, @bitCast(OP_TFORPREP)),
        };
    };
    _ = &forprep;
    const forloop = struct {
        const static: [2]OpCode = [2]OpCode{
            @as(c_uint, @bitCast(OP_FORLOOP)),
            @as(c_uint, @bitCast(OP_TFORLOOP)),
        };
    };
    _ = &forloop;
    var bl: BlockCnt = undefined;
    ...
    var fs: [*c]FuncState = ls.*.fs;
    _ = &fs;
    var prep: c_int = undefined;
    _ = &prep;
    var endfor: c_int = undefined;
    _ = &endfor;
    checknext(ls, TK_DO);
    prep = luaK_codeABx(fs, forprep.static[@as(c_uint, @intCast(isgen))], base, @as(c_uint, @bitCast(@as(c_int, 0))));
    enterblock(fs, &bl, @as(lu_byte, @bitCast(@as(i8, @truncate(@as(c_int, 0))))));
    adjustlocalvars(ls, nvars);
    luaK_reserveregs(fs, nvars);
    block(ls);
    leaveblock(fs);
    fixforjump(fs, prep, luaK_getlabel(fs), @as(c_int, 0));
    if (isgen != 0) {
        _ = luaK_codeABCk(fs, @as(c_uint, @bitCast(OP_TFORCALL)), base, @as(c_int, 0), nvars, @as(c_int, 0));
        luaK_fixline(fs, line);
    }
    endfor = luaK_codeABx(fs, forloop.static[@as(c_uint, @intCast(isgen))], base, @as(c_uint, @bitCast(@as(c_int, 0))));
    fixforjump(fs, endfor, prep + @as(c_int, 1), @as(c_int, 1));
    luaK_fixline(fs, line);
}
pub fn fornum(arg_ls: [*c]LexState, arg_varname: [*c]TString, arg_line: c_int) callconv(.c) void {
    var ls = arg_ls;
    _ = &ls;
    ...
    if (testnext(ls, @as(c_int, ',')) != 0) {
        ...
        luaK_int(fs, @as(c_int, @bitCast(@as(c_uint, fs.*.freereg))), @as(lua_Integer, @bitCast(@as(c_longlong, @as(c_int, 1)))));
        luaK_reserveregs(fs, @as(c_int, 1));
    }
    adjustlocalvars(ls, @as(c_int, 3));
    forbody(ls, base, line, @as(c_int, 1), @as(c_int, 0));
}
pub fn forlist(arg_ls: [*c]LexState, arg_indexname: [*c]TString) callconv(.c) void {
    var ls = arg_ls;
    _ = &ls;
    var indexname = arg_indexname;
    _ = &indexname;
    var fs: [*c]FuncState = ls.*.fs;
    _ = &fs;
    var e: expdesc = undefined;
    ...
    checknext(ls, TK_IN);
    line = ls.*.linenumber;
    adjust_assign(ls, @as(c_int, 4), explist(ls, &e), &e);
    adjustlocalvars(ls, @as(c_int, 4));
    marktobeclosed(fs);
    luaK_checkstack(fs, @as(c_int, 3));
    forbody(ls, base, line, nvars - @as(c_int, 4), @as(c_int, 1));
}
pub fn forstat(arg_ls: [*c]LexState, arg_line: c_int) callconv(.c) void {
    var ls = arg_ls;
    _ = &ls;
    var line = arg_line;
    _ = &line;
    var fs: [*c]FuncState = ls.*.fs;
    _ = &fs;
    var varname: [*c]TString = undefined;
    _ = &varname;
    var bl: BlockCnt = undefined;
    _ = &bl;
    enterblock(fs, &bl, @as(lu_byte, @bitCast(@as(i8, @truncate(@as(c_int, 1))))));
...
pub fn test_then_block(arg_ls: [*c]LexState, arg_escapelist: [*c]c_int) callconv(.c) void {
    var ls = arg_ls;
    _ = &ls;
    var escapelist = arg_escapelist;
    _ = &escapelist;
    var bl: BlockCnt = undefined;
    _ = &bl;
    var fs: [*c]FuncState = ls.*.fs;
...
pub fn ifstat(arg_ls: [*c]LexState, arg_line: c_int) callconv(.c) void {
    var ls = arg_ls;
    _ = &ls;
    var line = arg_line;
    _ = &line;
    var fs: [*c]FuncState = ls.*.fs;
    _ = &fs;
    var escapelist: c_int = -@as(c_int, 1);
    _ = &escapelist;
...
pub fn localfunc(arg_ls: [*c]LexState) callconv(.c) void {
    var ls = arg_ls;
    _ = &ls;
    ...
    var fs: [*c]FuncState = ls.*.fs;
    _ = &fs;
    var fvar: c_int = @as(c_int, @bitCast(@as(c_uint, fs.*.nactvar)));
    _ = &fvar;
    _ = new_localvar(ls, str_checkname(ls));
    adjustlocalvars(ls, @as(c_int, 1));
    body(ls, &b, @as(c_int, 0), ls.*.linenumber);
    localdebuginfo(fs, fvar).*.startpc = fs.*.pc;
}
pub fn getlocalattribute(arg_ls: [*c]LexState) callconv(.c) c_int {
    var ls = arg_ls;
    _ = &ls;
    if (testnext(ls, @as(c_int, '<')) != 0) {
        var attr: [*c]const u8 = @as([*c]u8, @ptrCast(@alignCast(&str_checkname(ls).*.contents[@as(usize, @intCast(0))])));
        _ = &attr;
        checknext(ls, @as(c_int, '>'));
        if (strcmp(attr, "const") == @as(c_int, 0)) return 1 else if (strcmp(attr, "close") == @as(c_int, 0)) return 2 else {
            {
                var fmtbuf: [512]u8 = undefined;
                const result = std.fmt.bufPrintZ(&fmtbuf, "unknown attribute '{s}'", .{
                    std.mem.span(@as([*:0]const u8, @ptrCast(attr))),
                }) catch "unknown attribute '?'";
                luaK_semerror(ls, lua_pushstring(ls.*.L, result));
            }
        }
    }
    return 0;
}
pub fn checktoclose(arg_fs: [*c]FuncState, arg_level: c_int) callconv(.c) void {
    var fs = arg_fs;
    _ = &fs;
    var level = arg_level;
    _ = &level;
    if (level != -@as(c_int, 1)) {
        marktobeclosed(fs);
        _ = luaK_codeABCk(fs, @as(c_uint, @bitCast(OP_TBC)), reglevel(fs, level), @as(c_int, 0), @as(c_int, 0), @as(c_int, 0));
    }
}
pub fn localstat(arg_ls: [*c]LexState) callconv(.c) void {
    var ls = arg_ls;
    _ = &ls;
    var fs: [*c]FuncState = ls.*.fs;
    _ = &fs;
    var toclose: c_int = -@as(c_int, 1);
    _ = &toclose;
    var @"var": [*c]Vardesc = undefined;
    _ = &@"var";
    var vidx: c_int = undefined;
    _ = &vidx;
    ...
    while (true) {
        ...
        getlocalvardesc(fs, vidx).*.vd.kind = @as(lu_byte, @bitCast(@as(i8, @truncate(kind))));
        if (kind == @as(c_int, 2)) {
            if (toclose != -@as(c_int, 1)) {
                luaK_semerror(ls, "multiple to-be-closed variables in local list");
            }
            toclose = @as(c_int, @bitCast(@as(c_uint, fs.*.nactvar))) + nvars;
        }
        nvars += 1;
        if (!(testnext(ls, @as(c_int, ',')) != 0)) break;
    }
    if (testnext(ls, @as(c_int, '=')) != 0) {
        nexps = explist(ls, &e);
    } else {
        e.k = @as(c_uint, @bitCast(VVOID));
        nexps = 0;
    }
    @"var" = getlocalvardesc(fs, vidx);
    if (((nvars == nexps) and (@as(c_int, @bitCast(@as(c_uint, @"var".*.vd.kind))) == @as(c_int, 1))) and (luaK_exp2const(fs, &e, &@"var".*.k) != 0)) {
        @"var".*.vd.kind = 3;
        adjustlocalvars(ls, nvars - @as(c_int, 1));
        fs.*.nactvar +%= 1;
    } else {
        adjust_assign(ls, nvars, nexps, &e);
        adjustlocalvars(ls, nvars);
...
pub fn funcname(arg_ls: [*c]LexState, arg_v: [*c]expdesc) callconv(.c) c_int {
    var ls = arg_ls;
    _ = &ls;
    var v = arg_v;
    _ = &v;
    var ismethod: c_int = 0;
    _ = &ismethod;
    singlevar(ls, v);
...
pub fn funcstat(arg_ls: [*c]LexState, arg_line: c_int) callconv(.c) void {
    var ls = arg_ls;
    _ = &ls;
    var line = arg_line;
    _ = &line;
    var ismethod: c_int = undefined;
    _ = &ismethod;
    var v: expdesc = undefined;
    _ = &v;
    var b: expdesc = undefined;
...
pub fn exprstat(arg_ls: [*c]LexState) callconv(.c) void {
    var ls = arg_ls;
    _ = &ls;
    var fs: [*c]FuncState = ls.*.fs;
    _ = &fs;
    var v: struct_LHS_assign = undefined;
    _ = &v;
    suffixedexp(ls, &v.v);
    if ((ls.*.t.token == @as(c_int, '=')) or (ls.*.t.token == @as(c_int, ','))) {
        v.prev = null;
        restassign(ls, &v, @as(c_int, 1));
    } else {
        var inst: [*c]Instruction = undefined;
        _ = &inst;
        {
            if (!(v.v.k == @as(c_uint, @bitCast(VCALL)))) {
                luaX_syntaxerror(ls, "syntax error");
            }
        }
        inst = &(blk: {
            const tmp = (&v.v).*.u.info;
            if (tmp >= 0) break :blk fs.*.f.*.code + @as(usize, @intCast(tmp)) else break :blk fs.*.f.*.code - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
        }).*;
        _ = blk: {
            const tmp = (inst.* & ~(~(~@as(Instruction, @bitCast(@as(c_int, 0))) << @intCast(@as(c_int, 8))) << @intCast((((@as(c_int, 0) + @as(c_int, 7)) + @as(c_int, 8)) + @as(c_int, 1)) + @as(c_int, 8)))) | ((@as(Instruction, @bitCast(@as(c_int, 1))) << @intCast((((@as(c_int, 0) + @as(c_int, 7)) + @as(c_int, 8)) + @as(c_int, 1)) + @as(c_int, 8))) & (~(~@as(Instruction, @bitCast(@as(c_int, 0))) << @intCast(@as(c_int, 8))) << @intCast((((@as(c_int, 0) + @as(c_int, 7)) + @as(c_int, 8)) + @as(c_int, 1)) + @as(c_int, 8))));
            inst.* = tmp;
            break :blk tmp;
        };
    }
}
pub fn retstat(arg_ls: [*c]LexState) callconv(.c) void {
    var ls = arg_ls;
    _ = &ls;
    var fs: [*c]FuncState = ls.*.fs;
    _ = &fs;
    ...
    _ = &nret;
    var first: c_int = luaY_nvarstack(fs);
    _ = &first;
    if ((block_follow(ls, @as(c_int, 1)) != 0) or (ls.*.t.token == @as(c_int, ';'))) {
        nret = 0;
    } else {
        nret = explist(ls, &e);
        if ((e.k == @as(c_uint, @bitCast(VCALL))) or (e.k == @as(c_uint, @bitCast(VVARARG)))) {
            luaK_setreturns(fs, &e, -@as(c_int, 1));
            if (((e.k == @as(c_uint, @bitCast(VCALL))) and (nret == @as(c_int, 1))) and !(fs.*.bl.*.insidetbc != 0)) {
                _ = blk: {
                    const tmp = ((blk_1: {
                    ...
                    }).* & ~(~(~@as(Instruction, @bitCast(@as(c_int, 0))) << @intCast(@as(c_int, 7))) << @intCast(@as(c_int, 0)))) | ((@as(Instruction, @bitCast(OP_TAILCALL)) << @intCast(0)) & (~(~@as(Instruction, @bitCast(@as(c_int, 0))) << @intCast(@as(c_int, 7))) << @intCast(@as(c_int, 0))));
                    (blk_1: {
                        const tmp_2 = (&e).*.u.info;
                        if (tmp_2 >= 0) break :blk_1 fs.*.f.*.code + @as(usize, @intCast(tmp_2)) else break :blk_1 fs.*.f.*.code - ~@as(usize, @bitCast(@as(isize, @intCast(tmp_2)) +% -1));
                    }).* = tmp;
    ...
    _ = testnext(ls, @as(c_int, ';'));
}
pub fn mainfunc(arg_ls: [*c]LexState, arg_fs: [*c]FuncState) callconv(.c) void {
    var ls = arg_ls;
    _ = &ls;
    var fs = arg_fs;
    _ = &fs;
    var bl: BlockCnt = undefined;
    _ = &bl;
    var env: [*c]Upvaldesc = undefined;
    _ = &env;
    open_func(ls, fs, &bl);
    setvararg(fs, @as(c_int, 0));
    env = allocupvalue(fs);
    env.*.instack = 1;
    env.*.idx = 0;
    env.*.kind = 0;
    env.*.name = ls.*.envn;
    _ = if (((@as(c_int, @bitCast(@as(c_uint, fs.*.f.*.marked))) & (@as(c_int, 1) << @intCast(@as(c_int, 5)))) != 0) and ((@as(c_int, @bitCast(@as(c_uint, env.*.name.*.marked))) & ((@as(c_int, 1) << @intCast(@as(c_int, 3))) | (@as(c_int, 1) << @intCast(@as(c_int, 4))))) != 0)) luaC_barrier_(ls.*.L, blk: {
        _ = @as(c_int, 0);
        break :blk &@as([*c]union_GCUnion, @ptrCast(@alignCast(fs.*.f))).*.gc;
    }, blk: {
        _ = @as(c_int, 0);
        break :blk &@as([*c]union_GCUnion, @ptrCast(@alignCast(env.*.name))).*.gc;
    }) else @as(c_int, 0);
    luaX_next(ls);
    statlist(ls);
    check(ls, TK_EOS);
...
pub const MAXVARS = @as(c_int, 200);
pub inline fn hasmultret(k: anytype) @TypeOf((k == VCALL) or (k == VVARARG)) {
    _ = &k;
    return (k == VCALL) or (k == VVARARG);
}
pub inline fn eqstr(a: anytype, b: anytype) @TypeOf(a == b) {
    _ = &a;
    _ = &b;
    return a == b;
}
pub const check_condition = @compileError("unable to translate C expr: unexpected token '{'");
// /src/cosmopolitan/third_party/lua/lparser.c:138:9
pub const new_localvarliteral = @compileError("unable to translate C expr: unexpected token ';'");
// /src/cosmopolitan/third_party/lua/lparser.c:224:9
pub inline fn enterlevel(ls: anytype) @TypeOf(luaE_incCstack(ls.*.L)) {
    _ = &ls;
    return luaE_incCstack(ls.*.L);
}
pub const leavelevel = @compileError("TODO postfix inc/dec expr");
// /src/cosmopolitan/third_party/lua/lparser.c:523:9
pub const UNARY_PRIORITY = @as(c_int, 12);
...
pub const LHS_assign = struct_LHS_assign;
