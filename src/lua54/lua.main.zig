...
pub fn print_version() callconv(.c) void {
    ...
    _ = blk: {
        _ = fwrite(@as(?*const anyopaque, @ptrCast("\n")), @sizeOf(u8), @as(usize, @bitCast(@as(isize, @as(c_int, 1)))), stdout);
...
pub export fn main(arg_argc: c_int, arg_argv: [*c][*c]u8) c_int {
    ...
    status = lua_pcallk(L, @as(c_int, 2), @as(c_int, 1), @as(c_int, 0), @as(lua_KContext, @bitCast(@as(isize, @as(c_int, 0)))), null);
