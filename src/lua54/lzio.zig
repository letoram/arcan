...
pub export fn luaZ_init(arg_L: [*c]lua_State, arg_z: [*c]ZIO, arg_reader: lua_Reader, arg_data: ?*anyopaque) void {
    var L = arg_L;
    _ = &L;
    var z = arg_z;
    _ = &z;
...
pub export fn luaZ_read(arg_z: [*c]ZIO, arg_b: ?*anyopaque, arg_n: usize) usize {
    var z = arg_z;
    _ = &z;
    var b = arg_b;
    _ = &b;
    ...
    while (n != 0) {
        ...
        _ = &m;
        if (z.*.n == @as(usize, @bitCast(@as(c_long, @as(c_int, 0))))) {
            if (luaZ_fill(z) == -@as(c_int, 1)) return n else {
                z.*.n +%= 1;
                z.*.p -= 1;
            }
        }
        m = if (n <= z.*.n) n else z.*.n;
        _ = memcpy(b, @as(?*const anyopaque, @ptrCast(z.*.p)), m);
        z.*.n -%= m;
        z.*.p += m;
        b = @as(?*anyopaque, @ptrCast(@as([*c]u8, @ptrCast(@alignCast(b))) + m));
        n -%= m;
    }
    return 0;
}
pub export fn luaZ_fill(arg_z: [*c]ZIO) c_int {
    var z = arg_z;
    _ = &z;
    var size: usize = undefined;
    _ = &size;
    var L: [*c]lua_State = z.*.L;
    _ = &L;
    var buff: [*c]const u8 = undefined;
    _ = &buff;
    _ = @as(c_int, 0);
    buff = z.*.reader.?(L, z.*.data, &size);
    _ = @as(c_int, 0);
    if ((buff == @as([*c]const u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) or (size == @as(usize, @bitCast(@as(c_long, @as(c_int, 0)))))) return -@as(c_int, 1);
    z.*.n = size -% @as(usize, @bitCast(@as(c_long, @as(c_int, 1))));
    z.*.p = buff;
    return @as(c_int, @bitCast(@as(c_uint, @as(u8, @bitCast((blk: {
        const ref = &z.*.p;
        const tmp = ref.*;
        ref.* += 1;
        break :blk tmp;
