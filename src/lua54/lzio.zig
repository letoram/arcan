...
pub export fn luaZ_read(arg_z: [*c]ZIO, arg_b: ?*anyopaque, arg_n: usize) usize {
    ...
    while (n != 0) {
        ...
        _ = &m;
        if (z.*.n == @as(usize, @bitCast(@as(isize, @as(c_int, 0))))) {
...
pub export fn luaZ_fill(arg_z: [*c]ZIO) c_int {
    ...
    _ = @as(c_int, 0);
    if ((buff == @as([*c]const u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) or (size == @as(usize, @bitCast(@as(isize, @as(c_int, 0)))))) return -@as(c_int, 1);
    z.*.n = size -% @as(usize, @bitCast(@as(isize, @as(c_int, 1))));
    z.*.p = buff;
