...
pub fn setpath(L: ?*lua_State, fieldname: [*c]const u8, envname: [*c]const u8, dft: [*c]const u8) callconv(.c) void {
    ...
    if ((path == null) or (noenv(L) != 0)) {
        ...
        luaL_buffinit(L, &b);
        if (path < dftmark) {
            luaL_addlstring(&b, path, @as(usize, @bitCast(@divExact(@as(isize, @bitCast(@intFromPtr(dftmark) -% @intFromPtr(path))), @sizeOf(u8)))));
            _ = blk: {
        ...
        if (dftmark < ((path + len) - @as(usize, 2))) {
            ...
            luaL_addlstring(&b, dftmark + @as(usize, 2), @as(usize, @bitCast(@divExact(@as(isize, @bitCast(@intFromPtr((path + len) - @as(usize, 2)) -% @intFromPtr(dftmark))), @sizeOf(u8)))));
        }
        luaL_pushresult(&b);
    }
...
pub fn loadfunc(L: ?*lua_State, filename: [*c]const u8, arg_modname: [*c]const u8) callconv(.c) c_int {
    ...
    if (mark != null) {
        ...
        openfunc = lua_pushlstring(L, modname, @as(usize, @bitCast(@divExact(@as(isize, @bitCast(@intFromPtr(mark) -% @intFromPtr(modname))), @sizeOf(u8)))));
        {
            var fmtbuf: [512]u8 = undefined;
...
pub fn searcher_Croot(L: ?*lua_State) callconv(.c) c_int {
    ...
    var stat: c_int = undefined;
    if (p == null) return 0;
    _ = lua_pushlstring(L, name, @as(usize, @bitCast(@divExact(@as(isize, @bitCast(@intFromPtr(p) -% @intFromPtr(name))), @sizeOf(u8)))));
