...
pub extern fn dlopen([*c]const u8, c_int) ?*anyopaque;
pub extern fn dlsym(?*anyopaque, [*c]const u8) ?*anyopaque;
pub extern fn dlclose(?*anyopaque) c_int;
pub extern fn cosmo_dlerror() [*c]u8;
pub extern fn cosmo_dlopen([*c]const u8, c_int) ?*anyopaque;
pub extern fn cosmo_dlsym(?*anyopaque, [*c]const u8) ?*anyopaque;
pub extern fn cosmo_dltramp(?*anyopaque) ?*anyopaque;
pub extern fn cosmo_dlclose(?*anyopaque) c_int;
...
pub export fn luaopen_package(L: ?*lua_State) callconv(.c) c_int {
    ...
    _ = blk: {
        _ = blk_1: {
            ...
            break :blk_1 lua_createtable(L, 0, @as(c_int, @bitCast(@as(c_uint, @truncate((@sizeOf([8]luaL_Reg) / @sizeOf(luaL_Reg)) -% @as(c_ulong, 1))))));
        };
        break :blk luaL_setfuncs(L, @as([*c]const luaL_Reg, @ptrCast(@alignCast(&pk_funcs[0]))), 0);
    };
    createsearcherstable(L);
    setpath(L, "path", "LUA_PATH", g_lua_path_default);
    setpath(L, "cpath", "LUA_CPATH", "/zip/.lua/?.so;/zip/.lua/loadall.so;./?.so");
    _ = lua_pushstring(L, "/\n;\n?\n!\n-\n");
    lua_setfield(L, -2, "config");
    _ = luaL_getsubtable(L, LUA_REGISTRYINDEX, "_LOADED");
    lua_setfield(L, -2, "loaded");
    _ = luaL_getsubtable(L, LUA_REGISTRYINDEX, "_PRELOAD");
    lua_setfield(L, -2, "preload");
    _ = lua_rawgeti(L, LUA_REGISTRYINDEX, @as(lua_Integer, 2));
    lua_pushvalue(L, -2);
    luaL_setfuncs(L, @as([*c]const luaL_Reg, @ptrCast(@alignCast(&ll_funcs[0]))), 1);
    lua_settop(L, -1 - 1);
    return 1;
}
...
pub const CLIBS: [*c]const u8 = "_CLIBS";
pub const voidf = ?*const fn () callconv(.c) void;
pub fn lsys_unloadlib(_: ?*anyopaque) callconv(.c) void {
}
pub fn lsys_load(L: ?*lua_State, _: [*c]const u8, _: c_int) callconv(.c) ?*anyopaque {
    _ = lua_pushstring(L, "dynamic libraries not enabled; check your Lua installation");
    return null;
}
pub fn lsys_sym(L: ?*lua_State, _: ?*anyopaque, _: [*c]const u8) callconv(.c) lua_CFunction {
    _ = lua_pushstring(L, "dynamic libraries not enabled; check your Lua installation");
    return null;
}
pub fn noenv(L: ?*lua_State) callconv(.c) c_int {
    var b: c_int = undefined;
    _ = lua_getfield(L, LUA_REGISTRYINDEX, "LUA_NOENV");
    b = lua_toboolean(L, -1);
    lua_settop(L, -1 - 1);
    return b;
}
pub fn setpath(L: ?*lua_State, fieldname: [*c]const u8, envname: [*c]const u8, dft: [*c]const u8) callconv(.c) void {
    var dftmark: [*c]const u8 = undefined;
    const nver: [*c]const u8 = blk: {
        var fmtbuf: [512]u8 = undefined;
        const result = std.fmt.bufPrintZ(&fmtbuf, "{s}{s}", .{
            std.mem.span(@as([*:0]const u8, @ptrCast(envname))),
            @as([*:0]const u8, "_5_4"),
        }) catch "?_5_4";
        break :blk lua_pushstring(L, result);
    };
    var path: [*c]const u8 = getenv(nver);
    if (path == null) {
        path = getenv(envname);
    }
    if ((path == null) or (noenv(L) != 0)) {
        _ = lua_pushstring(L, dft);
    } else if ((blk: {
        const tmp = strstr(path, ";;");
        dftmark = tmp;
        break :blk tmp;
    }) == null) {
        _ = lua_pushstring(L, path);
    } else {
        const len: usize = strlen(path);
        var b: luaL_Buffer = undefined;
        luaL_buffinit(L, &b);
        if (path < dftmark) {
            luaL_addlstring(&b, path, @as(usize, @bitCast(@divExact(@as(c_long, @bitCast(@intFromPtr(dftmark) -% @intFromPtr(path))), @sizeOf(u8)))));
            _ = blk: {
        ...
        if (dftmark < ((path + len) - @as(usize, 2))) {
            _ = blk: {
                _ = ((&b).*.n < (&b).*.size) or (luaL_prepbuffsize(&b, @as(usize, 1)) != null);
                break :blk blk_1: {
                    const tmp: u8 = ';';
            ...
            luaL_addlstring(&b, dftmark + @as(usize, 2), @as(usize, @bitCast(@divExact(@as(c_long, @bitCast(@intFromPtr((path + len) - @as(usize, 2)) -% @intFromPtr(dftmark))), @sizeOf(u8)))));
        }
        luaL_pushresult(&b);
    }
...
pub fn checkclib(L: ?*lua_State, path: [*c]const u8) callconv(.c) ?*anyopaque {
    var plib: ?*anyopaque = undefined;
    _ = lua_getfield(L, LUA_REGISTRYINDEX, CLIBS);
    _ = lua_getfield(L, -1, path);
    plib = lua_touserdata(L, -1);
    lua_settop(L, -2 - 1);
    return plib;
}
pub fn addtoclib(L: ?*lua_State, path: [*c]const u8, plib: ?*anyopaque) callconv(.c) void {
    _ = lua_getfield(L, LUA_REGISTRYINDEX, CLIBS);
    lua_pushlightuserdata(L, plib);
    lua_pushvalue(L, -1);
    lua_setfield(L, -3, path);
    lua_rawseti(L, -2, luaL_len(L, -2) + @as(lua_Integer, 1));
    lua_settop(L, -1 - 1);
}
pub fn gctm(L: ?*lua_State) callconv(.c) c_int {
    var n: lua_Integer = luaL_len(L, 1);
    while (n >= @as(lua_Integer, 1)) : (n -= 1) {
        _ = lua_rawgeti(L, 1, n);
...
pub fn lookforfunc(L: ?*lua_State, path: [*c]const u8, sym: [*c]const u8) callconv(.c) c_int {
    var reg: ?*anyopaque = checkclib(L, path);
    if (reg == null) {
        reg = lsys_load(L, path, @intFromBool(@as(c_int, @intCast(sym.*)) == @as(c_int, '*')));
        if (reg == null) return 1;
        addtoclib(L, path, reg);
    }
    if (@as(c_int, @intCast(sym.*)) == @as(c_int, '*')) {
        lua_pushboolean(L, 1);
        return 0;
    } else {
        const f: lua_CFunction = lsys_sym(L, reg, sym);
        if (f == null) return 2;
...
pub fn ll_loadlib(L: ?*lua_State) callconv(.c) c_int {
    const path: [*c]const u8 = luaL_checklstring(L, 1, null);
    const init: [*c]const u8 = luaL_checklstring(L, 2, null);
    const stat: c_int = lookforfunc(L, path, init);
    if (stat == 0) return 1 else {
        lua_pushnil(L);
        lua_rotate(L, -2, 1);
        _ = lua_pushstring(L, if (stat == 1) "absent" else "init");
        return 3;
    }
    return 0;
}
pub fn readable(filename: [*c]const u8) callconv(.c) c_int {
    const f: ?*FILE = fopen(filename, "r");
    if (f == null) return 0;
    _ = fclose(f);
    return 1;
}
pub fn getnextfilename(path: [*c][*c]u8, end: [*c]u8) callconv(.c) [*c]const u8 {
    var sep: [*c]u8 = undefined;
    var name: [*c]u8 = path.*;
    if (name == end) return null else if (@as(c_int, @intCast(name.*)) == @as(c_int, '\x00')) {
        name.* = ';';
        name += 1;
    }
    sep = strchr(name, @as(c_int, ';'));
    if (sep == null) {
        sep = end;
    }
    sep.* = '\x00';
...
pub fn pusherrornotfound(L: ?*lua_State, path: [*c]const u8) callconv(.c) void {
    var b: luaL_Buffer = undefined;
    luaL_buffinit(L, &b);
...
pub fn searchpath(L: ?*lua_State, arg_name: [*c]const u8, path: [*c]const u8, sep: [*c]const u8, dirsep: [*c]const u8) callconv(.c) [*c]const u8 {
    var name = arg_name;
    var buff: luaL_Buffer = undefined;
    var pathname: [*c]u8 = undefined;
    var endpathname: [*c]u8 = undefined;
    var filename: [*c]const u8 = undefined;
    if ((@as(c_int, @intCast(sep.*)) != @as(c_int, '\x00')) and (strchr(name, @as(c_int, @intCast(sep.*))) != null)) {
        name = luaL_gsub(L, name, sep, dirsep);
    }
    luaL_buffinit(L, &buff);
    luaL_addgsub(&buff, path, "?", name);
    _ = blk: {
        _ = ((&buff).*.n < (&buff).*.size) or (luaL_prepbuffsize(&buff, @as(usize, 1)) != null);
        break :blk blk_1: {
            const tmp = 0;
            (&buff).*.b[blk_2: {
                const ref = &(&buff).*.n;
    ...
    pathname = (&buff).*.b;
    endpathname = (pathname + (&buff).*.n) - @as(usize, 1);
    while ((blk: {
        const tmp = getnextfilename(&pathname, endpathname);
        filename = tmp;
        break :blk tmp;
    }) != null) {
        if (readable(filename) != 0) return lua_pushstring(L, filename);
    }
    luaL_pushresult(&buff);
    pusherrornotfound(L, lua_tolstring(L, -1, null));
    return null;
}
pub fn ll_searchpath(L: ?*lua_State) callconv(.c) c_int {
    const f: [*c]const u8 = searchpath(L, luaL_checklstring(L, 1, null), luaL_checklstring(L, 2, null), luaL_optlstring(L, 3, ".", null), luaL_optlstring(L, 4, "/", null));
...
pub fn findfile(L: ?*lua_State, name: [*c]const u8, pname: [*c]const u8, dirsep: [*c]const u8) callconv(.c) [*c]const u8 {
    var path: [*c]const u8 = undefined;
    _ = lua_getfield(L, LUA_REGISTRYINDEX - 1, pname);
    path = lua_tolstring(L, -1, null);
    if (path == null) {
        _ = luaL_error(L, "'package.%s' must be a string", pname);
    }
    return searchpath(L, name, path, ".", dirsep);
}
pub fn checkload(L: ?*lua_State, stat: c_int, filename: [*c]const u8) callconv(.c) c_int {
    if (stat != 0) {
        _ = lua_pushstring(L, filename);
        return 2;
    } else return luaL_error(L, "error loading module '%s' from file '%s':\n\t%s", lua_tolstring(L, 1, null), filename, lua_tolstring(L, -1, null));
    return 0;
}
pub fn searcher_Lua(L: ?*lua_State) callconv(.c) c_int {
    var filename: [*c]const u8 = undefined;
    const name: [*c]const u8 = luaL_checklstring(L, 1, null);
    filename = findfile(L, name, "path", "/");
    if (filename == null) return 1;
    return checkload(L, @intFromBool(luaL_loadfilex(L, filename, null) == 0), filename);
}
pub fn loadfunc(L: ?*lua_State, filename: [*c]const u8, arg_modname: [*c]const u8) callconv(.c) c_int {
    var modname = arg_modname;
    var openfunc: [*c]const u8 = undefined;
    var mark: [*c]const u8 = undefined;
    modname = luaL_gsub(L, modname, ".", "_");
    mark = strchr(modname, @as(c_int, '-'));
    if (mark != null) {
        var stat: c_int = undefined;
        openfunc = lua_pushlstring(L, modname, @as(usize, @bitCast(@divExact(@as(c_long, @bitCast(@intFromPtr(mark) -% @intFromPtr(modname))), @sizeOf(u8)))));
        {
            var fmtbuf: [512]u8 = undefined;
            const result = std.fmt.bufPrintZ(&fmtbuf, "luaopen_{s}", .{
                std.mem.span(@as([*:0]const u8, @ptrCast(openfunc))),
            }) catch "luaopen_?";
            openfunc = lua_pushstring(L, result);
        }
        stat = lookforfunc(L, filename, openfunc);
        if (stat != 2) return stat;
        modname = mark + @as(usize, 1);
    }
    {
        var fmtbuf: [512]u8 = undefined;
        const result = std.fmt.bufPrintZ(&fmtbuf, "luaopen_{s}", .{
            std.mem.span(@as([*:0]const u8, @ptrCast(modname))),
        }) catch "luaopen_?";
        openfunc = lua_pushstring(L, result);
    }
    return lookforfunc(L, filename, openfunc);
}
pub fn searcher_C(L: ?*lua_State) callconv(.c) c_int {
    const name: [*c]const u8 = luaL_checklstring(L, 1, null);
    const filename: [*c]const u8 = findfile(L, name, "cpath", "/");
    if (filename == null) return 1;
    return checkload(L, @intFromBool(loadfunc(L, filename, name) == 0), filename);
}
pub fn searcher_Croot(L: ?*lua_State) callconv(.c) c_int {
    var filename: [*c]const u8 = undefined;
    const name: [*c]const u8 = luaL_checklstring(L, 1, null);
    const p: [*c]const u8 = strchr(name, @as(c_int, '.'));
    var stat: c_int = undefined;
    if (p == null) return 0;
    _ = lua_pushlstring(L, name, @as(usize, @bitCast(@divExact(@as(c_long, @bitCast(@intFromPtr(p) -% @intFromPtr(name))), @sizeOf(u8)))));
    filename = findfile(L, lua_tolstring(L, -1, null), "cpath", "/");
    if (filename == null) return 1;
    if ((blk: {
        const tmp = loadfunc(L, filename, name);
        stat = tmp;
        break :blk tmp;
    }) != 0) {
        if (stat != 2) return checkload(L, 0, filename) else {
            {
                var fmtbuf: [512]u8 = undefined;
                const result = std.fmt.bufPrintZ(&fmtbuf, "no module '{s}' in file '{s}'", .{
                    std.mem.span(@as([*:0]const u8, @ptrCast(name))),
                    std.mem.span(@as([*:0]const u8, @ptrCast(filename))),
                }) catch "no module '?' in file '?'";
...
pub fn searcher_preload(L: ?*lua_State) callconv(.c) c_int {
    const name: [*c]const u8 = luaL_checklstring(L, 1, null);
    _ = lua_getfield(L, LUA_REGISTRYINDEX, "_PRELOAD");
    if (lua_getfield(L, -1, name) == 0) {
        {
            var fmtbuf: [512]u8 = undefined;
            const result = std.fmt.bufPrintZ(&fmtbuf, "no field package.preload['{s}']", .{
                std.mem.span(@as([*:0]const u8, @ptrCast(name))),
            }) catch "no field package.preload['?']";
            _ = lua_pushstring(L, result);
        }
        return 1;
    } else {
        _ = lua_pushstring(L, ":preload:");
        return 2;
    }
    return 0;
}
pub fn findloader(L: ?*lua_State, name: [*c]const u8) callconv(.c) void {
    var i: c_int = undefined;
    var msg: luaL_Buffer = undefined;
    if ((lua_getfield(L, LUA_REGISTRYINDEX - 1, "searchers") != 5)) {
        _ = luaL_error(L, "'package.searchers' must be a table");
    }
    luaL_buffinit(L, &msg);
    {
        i = 1;
        while (true) : (i += 1) {
            luaL_addstring(&msg, "\n\t");
            if ((lua_rawgeti(L, 3, @as(lua_Integer, @intCast(i))) == 0)) {
                lua_settop(L, -1 - 1);
                _ = blk: {
                    const ref = &(&msg).*.n;
                    ref.* -%= @as(usize, 2);
                ...
                luaL_pushresult(&msg);
                _ = luaL_error(L, "module '%s' not found:%s", name, lua_tolstring(L, -1, null));
            }
            _ = lua_pushstring(L, name);
            lua_callk(L, 1, 2, 0, null);
            if (lua_type(L, -2) == 6) return else if (lua_isstring(L, -2) != 0) {
                lua_settop(L, -1 - 1);
                luaL_addvalue(&msg);
            } else {
                lua_settop(L, -2 - 1);
...
pub fn ll_require(L: ?*lua_State) callconv(.c) c_int {
    const name: [*c]const u8 = luaL_checklstring(L, 1, null);
    lua_settop(L, 1);
    _ = lua_getfield(L, LUA_REGISTRYINDEX, "_LOADED");
    _ = lua_getfield(L, 2, name);
    if (lua_toboolean(L, -1) != 0) return 1;
    lua_settop(L, -1 - 1);
    findloader(L, name);
    lua_rotate(L, -2, 1);
    lua_pushvalue(L, 1);
...
pub const pk_funcs: [8]luaL_Reg = [8]luaL_Reg{
    luaL_Reg{
        .name = "loadlib",
        .func = &ll_loadlib,
    },
...
pub const ll_funcs: [2]luaL_Reg = [2]luaL_Reg{
    luaL_Reg{
        .name = "require",
        .func = &ll_require,
    },
...
pub fn createsearcherstable(L: ?*lua_State) callconv(.c) void {
    const searchers = struct {
        const static: [5]lua_CFunction = [5]lua_CFunction{
            &searcher_preload,
            &searcher_Lua,
            &searcher_C,
            &searcher_Croot,
    ...
    var i: c_int = undefined;
    lua_createtable(L, @as(c_int, @bitCast(@as(c_uint, @truncate((@sizeOf([5]lua_CFunction) / @sizeOf(lua_CFunction)) -% @as(c_ulong, 1))))), 0);
    {
        i = 0;
        while (searchers.static[@as(c_uint, @intCast(i))] != null) : (i += 1) {
            lua_pushvalue(L, -2);
            lua_pushcclosure(L, searchers.static[@as(c_uint, @intCast(i))], 1);
            lua_rawseti(L, -2, @as(lua_Integer, @bitCast(@as(c_longlong, i + 1))));
        }
    }
    lua_setfield(L, -2, "searchers");
}
pub fn createclibstable(L: ?*lua_State) callconv(.c) void {
    _ = luaL_getsubtable(L, LUA_REGISTRYINDEX, CLIBS);
    lua_createtable(L, 0, 1);
    lua_pushcclosure(L, &gctm, 0);
    lua_setfield(L, -2, "__gc");
    _ = lua_setmetatable(L, -2);
}
pub fn GetLuaPathDefault() callconv(.c) [*c]const u8 {
