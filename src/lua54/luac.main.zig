...
pub fn reader(arg_L: [*c]lua_State, arg_ud: ?*anyopaque, arg_size: [*c]usize) callconv(.c) [*c]const u8 {
    ...
    if ((blk: {
        ...
        size.* = @sizeOf([20]u8) -% @as(usize, @bitCast(@as(isize, @as(c_int, 1))));
        return "(function()end)();\n";
    } else {
        size.* = 0;
        return null;
...
pub fn writer(arg_L: [*c]lua_State, arg_p: ?*const anyopaque, arg_size: usize, arg_u: ?*anyopaque) callconv(.c) c_int {
    ...
    _ = &u;
    _ = &L;
    return @intFromBool((fwrite(p, size, @as(usize, @bitCast(@as(isize, @as(c_int, 1)))), @as(?*FILE, @ptrCast(u))) != @as(usize, @bitCast(@as(isize, @as(c_int, 1))))) and (size != @as(usize, @bitCast(@as(isize, @as(c_int, 0))))));
}
...
pub export fn main(arg_argc: c_int, arg_argv: [*c][*c]u8) c_int {
    ...
    if (lua_pcallk(L, @as(c_int, 2), @as(c_int, 0), @as(c_int, 0), @as(lua_KContext, @bitCast(@as(isize, @as(c_int, 0)))), null) != @as(c_int, 0)) {
        fatal(lua_tolstring(L, -@as(c_int, 1), null));
    }
    lua_close(L);
    return 0;
...
pub fn PrintString(arg_ts: [*c]const TString) callconv(.c) void {
    ...
    _ = &i;
    var n: usize = if (@as(c_int, @bitCast(@as(c_uint, ts.*.tt))) == (@as(c_int, 4) | (@as(c_int, 0) << @intCast(4)))) @as(usize, @bitCast(@as(usize, ts.*.shrlen))) else ts.*.u.lnglen;
    _ = &n;
    _ = printf("\"");
    {
        i = 0;
