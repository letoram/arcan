...
pub export fn luaS_hash(arg_str: [*c]const u8, arg_l: usize, arg_seed: c_uint) c_uint {
    var str = arg_str;
    _ = &str;
    var l = arg_l;
    _ = &l;
    var seed = arg_seed;
    _ = &seed;
    var h: c_uint = seed ^ @as(c_uint, @bitCast(@as(c_uint, @truncate(l))));
    _ = &h;
    while (l > @as(usize, @bitCast(@as(c_long, @as(c_int, 0))))) : (l -%= 1) {
        h ^= ((h << @intCast(5)) +% (h >> @intCast(2))) +% @as(c_uint, @bitCast(@as(c_uint, @as(lu_byte, @bitCast(str[l -% @as(usize, @bitCast(@as(c_long, @as(c_int, 1))))])))));
    }
    return h;
}
pub export fn luaS_hashlongstr(arg_ts: [*c]TString) c_uint {
    var ts = arg_ts;
    _ = &ts;
    _ = @as(c_int, 0);
    if (@as(c_int, @bitCast(@as(c_uint, ts.*.extra))) == @as(c_int, 0)) {
        var len: usize = ts.*.u.lnglen;
        _ = &len;
        ts.*.hash = luaS_hash(@as([*c]u8, @ptrCast(@alignCast(&ts.*.contents[@as(usize, @intCast(0))]))), len, ts.*.hash);
        ts.*.extra = 1;
    }
    return ts.*.hash;
}
pub export fn luaS_eqlngstr(arg_a: [*c]TString, arg_b: [*c]TString) c_int {
    var a = arg_a;
    _ = &a;
    var b = arg_b;
    _ = &b;
    var len: usize = a.*.u.lnglen;
    _ = &len;
    _ = @as(c_int, 0);
    return @intFromBool((a == b) or ((len == b.*.u.lnglen) and (memcmp(@as(?*const anyopaque, @ptrCast(@as([*c]u8, @ptrCast(@alignCast(&a.*.contents[@as(usize, @intCast(0))]))))), @as(?*const anyopaque, @ptrCast(@as([*c]u8, @ptrCast(@alignCast(&b.*.contents[@as(usize, @intCast(0))]))))), len) == @as(c_int, 0))));
}
pub export fn luaS_resize(arg_L: [*c]lua_State, arg_nsize: c_int) void {
    var L = arg_L;
    _ = &L;
    var nsize = arg_nsize;
    _ = &nsize;
    var tb: [*c]stringtable = &L.*.l_G.*.strt;
    _ = &tb;
    var osize: c_int = tb.*.size;
    _ = &osize;
    var newvect: [*c][*c]TString = undefined;
    _ = &newvect;
    if (nsize < osize) {
        tablerehash(tb.*.hash, osize, nsize);
    }
    newvect = @as([*c][*c]TString, @ptrCast(@alignCast(luaM_realloc_(L, @as(?*anyopaque, @ptrCast(tb.*.hash)), @as(usize, @bitCast(@as(c_long, osize))) *% @sizeOf([*c]TString), @as(usize, @bitCast(@as(c_long, nsize))) *% @sizeOf([*c]TString)))));
    if (__builtin_expect(@as(c_long, @intFromBool(@intFromBool(newvect == @as([*c][*c]TString, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) != @as(c_int, 0))), @as(c_long, @bitCast(@as(c_long, @as(c_int, 0))))) != 0) {
        if (nsize < osize) {
...
pub export fn luaS_clearcache(arg_g: [*c]global_State) void {
    var g = arg_g;
    _ = &g;
    ...
    {
        i = 0;
        while (i < @as(c_int, 53)) : (i += 1) {
            j = 0;
            while (j < @as(c_int, 2)) : (j += 1) {
                if ((@as(c_int, @bitCast(@as(c_uint, g.*.strcache[@as(c_uint, @intCast(i))][@as(c_uint, @intCast(j))].*.marked))) & ((@as(c_int, 1) << @intCast(@as(c_int, 3))) | (@as(c_int, 1) << @intCast(@as(c_int, 4))))) != 0) {
                    g.*.strcache[@as(c_uint, @intCast(i))][@as(c_uint, @intCast(j))] = g.*.memerrmsg;
                }
            }
        }
    }
}
pub export fn luaS_init(arg_L: [*c]lua_State) void {
    var L = arg_L;
    _ = &L;
    ...
    var tb: [*c]stringtable = &L.*.l_G.*.strt;
    _ = &tb;
    tb.*.hash = @as([*c][*c]TString, @ptrCast(@alignCast(luaM_malloc_(L, @as(c_ulong, @bitCast(@as(c_long, @as(c_int, 128)))) *% @sizeOf([*c]TString), @as(c_int, 0)))));
    tablerehash(tb.*.hash, @as(c_int, 0), @as(c_int, 128));
    tb.*.size = 128;
    g.*.memerrmsg = luaS_newlstr(L, "not enough memory", (@sizeOf([18]u8) / @sizeOf(u8)) -% @as(c_ulong, @bitCast(@as(c_long, @as(c_int, 1)))));
    luaC_fix(L, blk: {
        _ = @as(c_int, 0);
        break :blk &@as([*c]union_GCUnion, @ptrCast(@alignCast(g.*.memerrmsg))).*.gc;
    });
    {
        i = 0;
        while (i < @as(c_int, 53)) : (i += 1) {
...
pub export fn luaS_remove(arg_L: [*c]lua_State, arg_ts: [*c]TString) void {
    var L = arg_L;
    _ = &L;
    var ts = arg_ts;
    _ = &ts;
    var tb: [*c]stringtable = &L.*.l_G.*.strt;
    _ = &tb;
    var p: [*c][*c]TString = &(blk: {
        const tmp = blk_1: {
            _ = @as(c_int, 0);
            break :blk_1 @as(c_int, @bitCast(ts.*.hash & @as(c_uint, @bitCast(tb.*.size - @as(c_int, 1)))));
        };
        if (tmp >= 0) break :blk tb.*.hash + @as(usize, @intCast(tmp)) else break :blk tb.*.hash - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
    }).*;
    _ = &p;
    while (p.* != ts) {
        p = &p.*.*.u.hnext;
...
pub export fn luaS_newudata(arg_L: [*c]lua_State, arg_s: usize, arg_nuvalue: c_int) [*c]Udata {
    var L = arg_L;
    _ = &L;
    var s = arg_s;
    _ = &s;
    var nuvalue = arg_nuvalue;
    _ = &nuvalue;
    var u: [*c]Udata = undefined;
    _ = &u;
    var i: c_int = undefined;
    _ = &i;
    var o: [*c]GCObject = undefined;
    _ = &o;
    if (__builtin_expect(@as(c_long, @intFromBool(@intFromBool(s > ((if (@sizeOf(usize) < @sizeOf(lua_Integer)) ~@as(usize, @bitCast(@as(c_long, @as(c_int, 0)))) else @as(usize, @bitCast(@as(c_long, @truncate(@as(c_longlong, 9223372036854775807)))))) -% (if (nuvalue == @as(c_int, 0)) @offsetOf(struct_Udata0, "bindata") else @offsetOf(struct_Udata, "uv") +% (@sizeOf(UValue) *% @as(c_ulong, @bitCast(@as(c_long, nuvalue))))))) != @as(c_int, 0))), @as(c_long, @bitCast(@as(c_long, @as(c_int, 0))))) != 0) {
        luaM_toobig(L);
    }
    o = luaC_newobj(L, @as(c_int, 7) | (@as(c_int, 0) << @intCast(4)), (if (nuvalue == @as(c_int, 0)) @offsetOf(struct_Udata0, "bindata") else @offsetOf(struct_Udata, "uv") +% (@sizeOf(UValue) *% @as(c_ulong, @bitCast(@as(c_long, nuvalue))))) +% s);
    u = blk: {
        _ = @as(c_int, 0);
        break :blk &@as([*c]union_GCUnion, @ptrCast(@alignCast(o))).*.u;
    };
    u.*.len = s;
    u.*.nuvalue = @as(c_ushort, @bitCast(@as(c_short, @truncate(nuvalue))));
    u.*.metatable = null;
    {
        i = 0;
        while (i < nuvalue) : (i += 1) {
            _ = blk: {
                ...
                (&u.*.uv[@as(c_uint, @intCast(i))].uv).*.tt_ = tmp;
                break :blk tmp;
            };
        }
    }
...
pub export fn luaS_newlstr(arg_L: [*c]lua_State, arg_str: [*c]const u8, arg_l: usize) [*c]TString {
    var L = arg_L;
    _ = &L;
    var str = arg_str;
    _ = &str;
    var l = arg_l;
    _ = &l;
    if (l <= @as(usize, @bitCast(@as(c_long, @as(c_int, 40))))) return internshrstr(L, str, l) else {
        var ts: [*c]TString = undefined;
        _ = &ts;
        if (__builtin_expect(@as(c_long, @intFromBool(@intFromBool(l >= (((if (@sizeOf(usize) < @sizeOf(lua_Integer)) ~@as(usize, @bitCast(@as(c_long, @as(c_int, 0)))) else @as(usize, @bitCast(@as(c_long, @truncate(@as(c_longlong, 9223372036854775807)))))) -% @sizeOf(TString)) / @sizeOf(u8))) != @as(c_int, 0))), @as(c_long, @bitCast(@as(c_long, @as(c_int, 0))))) != 0) {
            luaM_toobig(L);
        }
        ts = luaS_createlngstrobj(L, l);
        _ = memcpy(@as(?*anyopaque, @ptrCast(@as([*c]u8, @ptrCast(@alignCast(&ts.*.contents[@as(usize, @intCast(0))]))))), @as(?*const anyopaque, @ptrCast(str)), l *% @sizeOf(u8));
        return ts;
    }
    return null;
}
pub export fn luaS_new(arg_L: [*c]lua_State, arg_str: [*c]const u8) [*c]TString {
    var L = arg_L;
    _ = &L;
    var str = arg_str;
    _ = &str;
    var i: c_uint = @as(c_uint, @bitCast(@as(c_uint, @truncate(@as(usize, @intCast(@intFromPtr(str))) & @as(usize, @bitCast(@as(c_ulong, @as(c_uint, 4294967295)))))))) % @as(c_uint, @bitCast(@as(c_int, 53)));
    _ = &i;
    var j: c_int = undefined;
    _ = &j;
    var p: [*c][*c]TString = @as([*c][*c]TString, @ptrCast(@alignCast(&L.*.l_G.*.strcache[i][@as(usize, @intCast(0))])));
    _ = &p;
    {
        j = 0;
        while (j < @as(c_int, 2)) : (j += 1) {
            if (strcmp(str, @as([*c]u8, @ptrCast(@alignCast(&(blk: {
                const tmp = j;
                if (tmp >= 0) break :blk p + @as(usize, @intCast(tmp)) else break :blk p - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
            }).*.*.contents[@as(usize, @intCast(0))])))) == @as(c_int, 0)) return (blk: {
                const tmp = j;
                if (tmp >= 0) break :blk p + @as(usize, @intCast(tmp)) else break :blk p - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
            }).*;
        }
    ...
    p[@as(c_uint, @intCast(@as(c_int, 0)))] = luaS_newlstr(L, str, strlen(str));
    return p[@as(c_uint, @intCast(@as(c_int, 0)))];
}
pub export fn luaS_createlngstrobj(arg_L: [*c]lua_State, arg_l: usize) [*c]TString {
    var L = arg_L;
    _ = &L;
    var l = arg_l;
    _ = &l;
    var ts: [*c]TString = createstrobj(L, l, @as(c_int, 4) | (@as(c_int, 1) << @intCast(4)), L.*.l_G.*.seed);
    _ = &ts;
    ts.*.u.lnglen = l;
    return ts;
}
...
pub fn tablerehash(arg_vect: [*c][*c]TString, arg_osize: c_int, arg_nsize: c_int) callconv(.c) void {
    var vect = arg_vect;
    _ = &vect;
    var osize = arg_osize;
    _ = &osize;
    ...
    _ = &i;
    {
        ...
        while (i < nsize) : (i += 1) {
            (blk: {
                const tmp = i;
                if (tmp >= 0) break :blk vect + @as(usize, @intCast(tmp)) else break :blk vect - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
            }).* = null;
        }
    }
    {
        ...
        while (i < osize) : (i += 1) {
            ...
            while (p != null) {
                var hnext: [*c]TString = p.*.u.hnext;
                _ = &hnext;
                var h: c_uint = @as(c_uint, @bitCast(blk: {
                    _ = @as(c_int, 0);
                    break :blk @as(c_int, @bitCast(p.*.hash & @as(c_uint, @bitCast(nsize - @as(c_int, 1)))));
                }));
                _ = &h;
                p.*.u.hnext = vect[h];
                vect[h] = p;
...
pub fn createstrobj(arg_L: [*c]lua_State, arg_l: usize, arg_tag: c_int, arg_h: c_uint) callconv(.c) [*c]TString {
    var L = arg_L;
    _ = &L;
    var l = arg_l;
    _ = &l;
    ...
    var o: [*c]GCObject = undefined;
    _ = &o;
    var totalsize: usize = undefined;
    _ = &totalsize;
    totalsize = @offsetOf(struct_TString, "contents") +% ((l +% @as(usize, @bitCast(@as(c_long, @as(c_int, 1))))) *% @sizeOf(u8));
    ...
    ts.*.hash = h;
    ts.*.extra = 0;
    @as([*]u8, @ptrCast(&ts.*.contents))[l] = '\x00';
    return ts;
}
pub fn growstrtab(arg_L: [*c]lua_State, arg_tb: [*c]stringtable) callconv(.c) void {
    var L = arg_L;
    _ = &L;
    var tb = arg_tb;
    _ = &tb;
    if (__builtin_expect(@as(c_long, @intFromBool(@intFromBool(tb.*.nuse == @as(c_int, 2147483647)) != @as(c_int, 0))), @as(c_long, @bitCast(@as(c_long, @as(c_int, 0))))) != 0) {
        luaC_fullgc(L, @as(c_int, 1));
        if (tb.*.nuse == @as(c_int, 2147483647)) {
            luaD_throw(L, @as(c_int, 4));
        }
    }
    if (tb.*.size <= @divTrunc(@as(c_int, @bitCast(if (@as(usize, @bitCast(@as(c_long, @as(c_int, 2147483647)))) <= (~@as(usize, @bitCast(@as(c_long, @as(c_int, 0)))) / @sizeOf([*c]TString))) @as(c_uint, @bitCast(@as(c_int, 2147483647))) else @as(c_uint, @bitCast(@as(c_uint, @truncate(~@as(usize, @bitCast(@as(c_long, @as(c_int, 0)))) / @sizeOf([*c]TString))))))), @as(c_int, 2))) {
        luaS_resize(L, tb.*.size * @as(c_int, 2));
    }
}
pub fn internshrstr(arg_L: [*c]lua_State, arg_str: [*c]const u8, arg_l: usize) callconv(.c) [*c]TString {
    var L = arg_L;
    _ = &L;
    var str = arg_str;
    _ = &str;
    ...
    var g: [*c]global_State = L.*.l_G;
    _ = &g;
    var tb: [*c]stringtable = &g.*.strt;
    _ = &tb;
    var h: c_uint = luaS_hash(str, l, g.*.seed);
    _ = &h;
    var list: [*c][*c]TString = &(blk: {
        const tmp = blk_1: {
            _ = @as(c_int, 0);
            break :blk_1 @as(c_int, @bitCast(h & @as(c_uint, @bitCast(tb.*.size - @as(c_int, 1)))));
        };
        if (tmp >= 0) break :blk tb.*.hash + @as(usize, @intCast(tmp)) else break :blk tb.*.hash - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
    }).*;
    _ = &list;
    ...
    {
        ts = list.*;
        while (ts != @as([*c]TString, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) : (ts = ts.*.u.hnext) {
            if ((l == @as(usize, @bitCast(@as(c_ulong, ts.*.shrlen)))) and (memcmp(@as(?*const anyopaque, @ptrCast(str)), @as(?*const anyopaque, @ptrCast(@as([*c]u8, @ptrCast(@alignCast(&ts.*.contents[@as(usize, @intCast(0))]))))), l *% @sizeOf(u8)) == @as(c_int, 0))) {
                if ((@as(c_int, @bitCast(@as(c_uint, ts.*.marked))) & (@as(c_int, @bitCast(@as(c_uint, g.*.currentwhite))) ^ ((@as(c_int, 1) << @intCast(@as(c_int, 3))) | (@as(c_int, 1) << @intCast(@as(c_int, 4)))))) != 0) {
                    _ = blk: {
                        const ref = &ts.*.marked;
                        ref.* ^= @as(lu_byte, @bitCast(@as(i8, @truncate((@as(c_int, 1) << @intCast(@as(c_int, 3))) | (@as(c_int, 1) << @intCast(@as(c_int, 4)))))));
                        break :blk ref.*;
                    };
                }
                return ts;
    ...
    ts = createstrobj(L, l, @as(c_int, 4) | (@as(c_int, 0) << @intCast(4)), h);
    _ = memcpy(@as(?*anyopaque, @ptrCast(@as([*c]u8, @ptrCast(@alignCast(&ts.*.contents[@as(usize, @intCast(0))]))))), @as(?*const anyopaque, @ptrCast(str)), l *% @sizeOf(u8));
    ts.*.shrlen = @as(lu_byte, @bitCast(@as(u8, @truncate(l))));
    ts.*.u.hnext = list.*;
    list.* = ts;
    tb.*.nuse += 1;
    return ts;
...
pub const MAXSTRTB = @compileError("unable to translate C expr: unexpected token ')'");
// /src/cosmopolitan/third_party/lua/lstring.c:46:9
