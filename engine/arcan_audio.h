...
/*
 * this one is shady at best, patchwork to get
 * audio - movie deps. allover the place coupled with
 * openAL headers, it only works assuming that
 * ALuint - unsigned match somehow (or is convertible)
 * if something bugs, suspect buffer arg.
 */
typedef arcan_errc(*arcan_afunc_cb)(struct arcan_aobj* aobj,
	arcan_aobj_id id, unsigned buffer, void* tag);

/* this callback is checked prior to buffering data
 * in OpenAL, which, in their infinite wisdom,
 * does not supply any aggregated means of gathering output
 * or being used as a resampler (or dummy input device
 * for that matter). As the ffmpeg/libav- cruft is not
 * allowed anywhere near the main process,  we're left with
 * this sorry hack -> grab buffers, put on a tag+len+data
 * and have the frameserver split the buffer into separate
 * streams and perform samplerate conversion / mixing there.
 */
 ...
 * OpenAL implementation might just freak out
 * and in stead botch the buffering..
 */
...
/* audiobuffers may need to be
	requeued / processed quicker than tickrate */
 ...
 * primarily used to grab or patch data fed to a
 * specific audio object for recording etc.
...
/*
 * This function is sed when trying to salvage a partly broken system
 * where the frameservers are the only ones allowed on the ark.
 */
