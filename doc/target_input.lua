...
--
-- universal:(subid, devid, unkind) optional:(ptsd, gesture, label).
-- The *devid* and *subid* fields are used to distinguish between different
-- devices and subaxes or buttons. The *unkind* field indicates the type of the
...
-- following: 'touchy', 'verytouchy', 'clickies', 'dblclickies', 'dragging',
-- 'swipe-left', 'just-a-fling', 'pressure', 'release', 'in-drag', 'drop',
-- 'openpincher', 'closepincher', 'revolve'.
...
--
-- If *unkind* is set to *touch* the required additional fields are:
-- pressure (0..1, linear scale), size (px), x (px) and y (px)
--
-- If *unkind* is set to *analog* the required additional fields are:
...
--
-- If *unkind* is set to *analog* the possible additional fields are:
...
--
-- If *unkind* is set to *digital*, the required additional fields are:
...
-- OS specific code).
--
-- If *unkind* is set to *eyes*, the required additional fields are:
...
-- If *pirate* is also set to true, the blink_left or blink_right are
-- considered replaced with an eyepatch and will be ignored. When sending
-- such an event *pegleg* should also be set to "yarrr".
--
