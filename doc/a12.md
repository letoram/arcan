...
Mode=1 (sigupd) MUST only ever be used by the client end. It is used to prove
metadata ownership for indexing and binary annotation. The kpub is the key
present in the metadata to be updated or retrieved. The ksig is an Ed25519
signature of the NONCE provided as part of the command header.
...
MUST be:
    ...
    kpub=base64(public-key-for-signature)
    sig=base64(signature of kpub over mhash)
    mhash=base64(BLAKE3 hash of the remainder of the metadata)
...
The state field is set to 0 if the entity has been added, and 1 if it has been
removed.
...
pseurandom number generator for generating the secret. The single exception is
if the entry to be opened is a referential link to a different directory server
that may not support an authentication secret. In that case the secret will be
set to 'SETECASTRONOMY', matching the a12 connection default.
...
The reserved '.ctrl' name is resversed for accessing the directory server side
