...
display server API and IPC system, [SHMIF][SHMIF], used by the
[Arcan project](ARCAN). To achieve this it adds extensions for supporting
...
Related protocols: [Remote Framebuffer, SSH,
...
[X25519][x25519] for public key. [ChaCha][CHACHA20] for stream cipher.
[Blake3][BLAKE3] used for MAC construction, key derivation, hashing for
cache management and integrity. Zstd[ZSTD] for generic compression.

It is also RECOMMENDED that [h264][H264] is present for video compression, but
an implementation MUST provide work-arounds for their absence.
...
It is a [HKDF][1] style scheme, but using [BLAKE3][2]
...
The reduced 8-round variant of [CHACHA](CHACHA20) as per the recommendations in
[Too Much Crypto][TOOMUCHCRYPTO]. This is done in order to allow a lower tier
of hardware without acceleration to still get reasonable throughput.
...
Commands](#target-commands) the event model.
...
The symbol is a segment type relative lookup table index. It is RECOMMENDED that
the default table used is that of [SDL2](SDL2-CODES) due to the range of platforms
it has been verified against. It is SUGGESTED that for Segment types e.g WAYLAND
and X11, the `<X11/keysymdef.h>` table is used as per [XLIBREF](XLIBREF)
...
Target events authoritate instructions flowing from sink to source. Their
...
level MUST be one of:
      ...
      0 : soft, 'return content to as close to initial state as possible'
      1 : hard, 'treat as soft, but also renegotiate any additional resources
                 such as windows and fonts'.
      2 : recovery, 'treat as hard, but assume any and all previously
      accumulated state has been lost'.
      3 : reconnect, 'treat as recovery, but assume that content preferences
...
The mode MUST be one of:
    0 : relative timestamp value is relative to the current content position
        and the value is in discrete monotonic ticks on some local reference
        clock.
    1 : absolute position with 0 meaning the start of the stream, and 1 the
        end.
...
The ATTENUATE event MAY be sent to a source to request that the input gain
on any audio presented on the channel SHOULD be lowered to the gain value
(within 0..1 range) before being passed as ASTREAM packets.
...
If there is an accompanying Font file in a format supported by
[RFC8081](RFC8081) it should be transmitted through a BINARY-STREAM command and
BINARY packets directly following the event.
...
8. Source performs local window system dependent operation to access the
   software to share.
9. Sink provides the initial set of target events, terminating with an ACTIVATE
   event.
...
[RFC8081]: https://datatracker.ietf.org/doc/html/rfc6143 "The 'font'
