...
pub fn __init() void {
    ...
    var cmds = .{
        "builtin dev",
        "write /tmp/eq_c1.txt one-two-three",
        "edit /tmp/eq_c1.txt one ONE",
        "read /tmp/eq_c1.txt",
        "edit /tmp/eq_c1.txt %w+ X",
        "read /tmp/eq_c1.txt",
    };
    var SLOT_BASE = 60;
    var SLOT_GAP = 30;
        ...
        for (cmds, 0..) |cmd, i| {
            ...
            if ((_tick == at) and !fired[key]) {
                ...
                if (lash_vid and valid_vid(lash_vid)) {
                    tag(string.format("slot=%d:start:%s", i, cmd));
                    send_line(lash_vid, cmd);
                }
            }
        }
        var END_TICK = START + @intCast(cmds.len) * SLOT_GAP + 200;
