...
int arcan_lua_scaleimage2(lua_State* ctx)
{
    ...
    else
    {
        ...
        else if (neww > 0.0001 && newh < 0.0001)
...
int arcan_lua_scaleimage(lua_State* ctx)
{
    ...
    else if (desw > 0.0001 && desh < 0.0001)
...
void arcan_lua_pushevent(lua_State* ctx, arcan_event* ev)
{
    ...
    else if (ev->category == EVENT_VIDEO)
    {
        ...
        case EVENT_VIDEO_EXPIRE  :
            arcan_lua_tblstr(ctx, "kind", "expired", top);
            evmsg = "video_event(expire)";
            break;
        case EVENT_VIDEO_SCALED  :
            arcan_lua_tblstr(ctx, "kind", "scaled", top);
            evmsg = "video_event(scale)";
            break;
        case EVENT_VIDEO_MOVED   :
            arcan_lua_tblstr(ctx, "kind", "moved", top);
            evmsg = "video_event(move))";
            break;
        case EVENT_VIDEO_BLENDED :
            arcan_lua_tblstr(ctx, "kind", "blended", top);
            evmsg = "video_event(blend)";
            break;
        case EVENT_VIDEO_ROTATED :
            arcan_lua_tblstr(ctx, "kind", "rotated", top);
            evmsg = "video_event(rotate)";
            break;
    ...
    else if (ev->category == EVENT_AUDIO && arcan_lua_grabthemefunction(ctx, "audio_event"))
    {
        ...
        switch (ev->kind)
        {
        case EVENT_AUDIO_BUFFER_UNDERRUN:
            arcan_lua_tblstr(ctx, "kind", "audio buffer underrun", top);
            break;
        case EVENT_AUDIO_GAIN_TRANSFORMATION_FINISHED:
            arcan_lua_tblstr(ctx, "kind", "gain transformed", top);
            break;
        case EVENT_AUDIO_PLAYBACK_FINISHED:
            arcan_lua_tblstr(ctx, "kind", "playback finished", top);
            break;
        case EVENT_AUDIO_PLAYBACK_ABORTED:
            arcan_lua_tblstr(ctx, "kind", "playback aborted", top);
            break;
        case EVENT_AUDIO_OBJECT_GONE:
            arcan_lua_tblstr(ctx, "kind", "gone", top);
            break;
        default:
...
int arcan_lua_targetlaunch_capabilities(lua_State* ctx)
{
    ...
    if (resourcestr)
    {
        ...
        if (use_loader(resourcestr))
        {
            ...
            arcan_lua_tblbool(ctx, "dynamic_input", true, top);
        ...
        else
        {
            ...
            arcan_lua_tblbool(ctx, "dynamic input", false, top);
